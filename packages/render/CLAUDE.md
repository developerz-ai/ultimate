...
| `RouteEntry.islands` | filled from `config.islands` at registration. It was `input.islands ?? []` and nothing ever passed `islands`, so `routeJsBytes`'s "what registration declared" half read nothing on every route in the framework's history. |
