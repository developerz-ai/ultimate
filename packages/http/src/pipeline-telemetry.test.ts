// The telemetry a request emits: its root span, the metrics the deploy chart scales on, and the
// report a server fault reaches. Split from `pipeline.test.ts` at the 500-line ceiling — that file
// asks what the pipeline DOES, this one asks what it reports while doing it.
