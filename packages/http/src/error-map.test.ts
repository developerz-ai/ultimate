...
describe('storage states the caller can act on', () => {
  ...
  // Same argument: promoting a key that is not a pending upload is a state conflict the caller
  // caused (a double promote, a key it never uploaded) — never "the server broke".
  test('promoting a key that is not pending is a 409', () => {
    expect(statusFor('X_STORAGE_NOT_PENDING')).toBe(409);
  });
});

describe('job states the caller can act on', () => {
  // A requeue of a job that is still live — the admin panel's retry button, `x jobs retry` served
  // over HTTP — is a conflict with the job's state, never a server fault that pages on-call.
  test('requeueing a live job is a 409', () => {
    expect(statusFor('X_JOB_NOT_REQUEUEABLE')).toBe(409);
  });

  // Declaration-time, so no request answers with it; the row exists because the table is closed.
  test('an invalid job declaration is the 500 it always was, now by row', () => {
    expect(ERROR_STATUS.X_JOB_DECLARATION_INVALID).toBe(500);
  });
});
