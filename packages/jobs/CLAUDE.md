...
pass through `@ultimat3/db`'s statement observer today, with no `attribution`, because nothing in
this package threads the `{entity, op}` pair `runOn`/`statement()` accept. That visibility is a
