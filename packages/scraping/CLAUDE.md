...
| a REDIRECT is a request nobody screened, so this leg follows it itself | `http.ts` sends `redirect: 'manual'` and walks the chain hop by hop (`http-redirect.ts`), re-asking `interceptVerdict`, `RobotsGate.assertAllowed`, `pace` and `cookieHeaderFor` for **every** hop, bounded by `MAX_REDIRECT_HOPS` (`X_SCRAPE_REDIRECT_LOOP`, terminal). Until 2026-09 the call carried the platform default: an allow-listed endpoint answering `302 → http://169.254.169.254/…` read the metadata service THROUGH the allow list, the gates having all been asked about the URL the caller wrote — and `res.url`, `network.push` and the robots read all named that URL, so nothing in the run said where the body came from. `responseOver` and the ring now carry the FINAL url. The CDP leg never had the hole, because interception fires per hop there, and this file's header claims parity with it (`http.test.ts`). **Two halves of that takeover were the platform's and had to be re-performed here, `As of 2026-09-06`:** a credential the caller set is dropped at the first CROSS-ORIGIN hop and stays dropped for the rest of the chain (`authorization`, `proxy-authorization`, a hand-written `cookie` — the fetch standard DELETES the header rather than re-deciding, so `A -> B -> A` does not hand the bearer back), and a `301`/`302` rewrites a `POST` and nothing else — "everything but GET/HEAD loses its body" turned a `PUT` into a bodyless `GET`, which is the caller's write silently not happening, answered 200 by a URL that stored nothing (`http-redirect-follow.test.ts`, `http-redirect.test.ts`) |
| a `javascript:` URL is refused, not treated as hostless | `hosts.ts`'s `REFUSED_SCHEMES`. It shared a line with `about:`/`data:`/`blob:` — all four have no host, and that is the whole resemblance: the other three are inert content, this one executes in the current document's origin with the session's cookies. Deleting it from the hostless set is NOT sufficient on its own, which is what the test pins: `javascript:` is not a special scheme, so `javascript://api.test/%0afetch('/admin')` parses with hostname `api.test` and an allow list naming that host would have MATCHED it (`hosts.test.ts`) |
...
| a theme picture is taken by emulating the PREFERENCE, never by setting an attribute | `page.colorScheme(scheme)` → `ScrapeTarget.setColorScheme` → CDP `Emulation.setEmulatedMedia`'s `prefers-color-scheme` (`color-scheme.ts`, `cdp-target.ts`). Setting `data-theme` on the document models the OUTCOME of a theme decision and the component owns that: one resolving `'system'` deletes or overwrites the attribute on mount, AFTER the document is parsed, so the harness is silently overruled. Measured on `examples/dummy` (#338) — `x shot --island` photographed every state in both themes and `<state>-light.png` and `<state>-dark.png` came back BYTE-IDENTICAL, same md5, out of two addresses that really did serve `data-theme="light"` and `data-theme="dark"`. REQUIRED on `ScrapeTarget` where `CdpPageLike.emulateMediaFeatures` is optional, for `setOfflineMode`'s reason, and the launcher without it is `X_NOT_IMPLEMENTED` by name. **ACCEPTED offline where `setOfflineMode` is REFUSED**, which is the one asymmetry between them: an offline *assertion* is reachable on a driver that answers content, and a colour preference has none — the only thing it could be wrong about is a picture, so `html-target.ts` answers different deterministic bytes per scheme exactly as it already does per `clip`, and a fake returning one constant for both themes is what let the defect ship. `'no-preference'` is a CLEAR and not a third value — `colorSchemeFeatures` sends CDP's EMPTY feature list, its own reset, because an explicit `prefers-color-scheme: no-preference` is an OVERRIDE. Measured on Chrome 150 headless: the two are indistinguishable on a machine with no preference and diverge on one that has a real preference, where the override forces the light answer and the reset gives the machine's own back |
