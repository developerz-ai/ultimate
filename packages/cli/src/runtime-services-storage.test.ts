...
// rather than after it. Split from `runtime-services.test.ts` at the 500-line ceiling: mail and the CDN
...
import { startStorage } from './runtime-services';
...
describe('unit · the storage binding a process boots with', () => {
  test('an unwritable embedded root is X_STORAGE_UNWRITABLE, not a bare EROFS', () => {
    ...
    try {
      ...
      const failure = (() => {
        try {
          return startStorage(services, { ULTIMATE_ENV: 'development' });
        } catch (error) {
          return error;
        }
      })();
  ...
  // A boot that names NO environment is not a development one. It read as `development` —
  // `isLocal`'s own fallback — and signed with the published key: the process that forgot to say
  // was exactly the one handed the dev secret.
  test('an env naming no environment at all is refused as production', () => {
    const root = mkdtempSync(join(tmpdir(), 'x-storage-env-none-'));
    const services = servicesAt(root, `file://${join(root, 'storage')}`, 'embedded');
    const failure = (() => {
      try {
    ...
    expect((failure as UltimateError).code).toBe('X_ENV_MISSING');
