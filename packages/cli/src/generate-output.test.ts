...
describe('unit · what x g emits', () => {
  ...
  test('the action generator writes the action, its test and the feature error type', () => {
    ...
    // are `contract`, and the gate types a test by its filename.
    expect(paths).toContain('apps/web/app/invoice/actions/publish-invoice.test.ts');
    expect(paths).toContain('apps/web/app/invoice/actions/publish-invoice.contract.test.ts');
  ...
  test('the action test pins the policy denial branch, through the action itself', () => {
    const files = generate({ kind: 'action', name: 'publish-invoice', feature: 'invoice' });
    const unit = files.find((file) => file.path.endsWith('publish-invoice.test.ts'));
    const testFile = files.find((file) => file.path.endsWith('publish-invoice.contract.test.ts'));
    expect(unit?.contents).toContain('toRejectInput');
  ...
  test('a live query is generated bounded and ordered, and its test says so', () => {
    ...
    // `.live.test.ts`: the wrapper is `liveTest`, so the `live` step is the one that selects it.
    const testFile = files.find((file) => file.path.endsWith('invoice-list.live.test.ts'));
