...
describe('unit · x mcp tools', () => {
  test('prints the framework catalog, not a list the CLI keeps', async () => {
    ...
    expect(result.summary).toBe('mcp none serving 15 tools');
...
describe('unit · x mcp serve --transport http', () => {
  ...
  test('the minted token is reported so an agent reads it from --json', () => {
    ...
    expect(server.result.summary).toBe('mcp http serving 15 tools');
  });
  ...
  test('the minted token lists all 15 tools', async () => {
 ...
 * directly", and the command's result — `✓ mcp stdio serving 15 tools`, rendered by `dispatch`
...
describe('unit · x mcp serve --transport stdio leaves stdout to the protocol', () => {
  test('the session result is addressed to stderr, and says what it served', () => {
    const result = stdioResult(15);
    expect(result.stream).toBe('stderr');
    expect(result.summary).toBe('mcp stdio serving 15 tools');
    expect(result.data).toEqual({ transport: 'stdio', tools: 15 });
  });
