...
afterEach(async () => {
  clearRoutes();
  // The stylesheet registry is process-global, and this file registers into it.
  clearStylesheets();
  await rm(ROOT, { recursive: true, force: true });
});
...
describe('x build --target static', () => {
  ...
  // owed the number `budget.js` no longer charges. It was measured off `/x-sw-register.js` ON
  // DISK while that file was still written LAST, beside `sw.js` — which is the run-order defect
  // the `jsBytes` split had just removed, alive again one field over: 0 on a clean output
  // directory because the file did not exist when it was weighed, and the PREVIOUS build's copy
  // on a reused one. Both cases are below, because a fix that closes one and not the other is the
  // same bug with a narrower window.
  //
  // The repair is the order, not the reader: the registration is two constants and a string, so
  // it is on disk before the first render. `sw.js` still comes last — its precache manifest is
  // built from the rendered documents' content hashes, which is a real dependency.
  // ------------------------------------------------------------------------------------------
...
// A static export is served with no process behind it, so every URL a document names has to be a
// file in the artifact. This is the promise `favicon.ico`, the icons and the island chunks already
// keep; the surface stylesheet joined them on 2026-09-06, when the CSS stopped being inline.
describe('the surface stylesheet is in the artifact the documents name', () => {
  test('the document links a file the export contains', async () => {
    registerRoute({ file: 'apps/web/site/page.tsx', config: staticRoute });
    loadStylesheet(join(ROOT, 'apps/web/site/page.module.scss'), '.hero{color:red}');
    const out = join(ROOT, 'static');
    const report = await prerenderSite({ root: ROOT, out, origin: 'https://example.test' });
    ...
    // The file itself, at the path the document spells — a `<link>` to a 404 is an unstyled page
    // on a CDN and a green build.
    expect(await Bun.file(join(out, href.slice(1))).text()).toContain('color:red');
