...
export function generatedClause(column: ColumnDescriptionLike): string {
  ...
  // The expression is SPLICED, so it is screened for a second command first — the rule
  // `declaredChecks` (`check-ddl.ts`) applies to a CHECK's predicate, on the same seam and with
  // the same lexer, so a `;` inside a string literal stays data. Measured before it:
  // `generated always as (upper(title)); drop table users; --) stored`.
  const commands = statementsOf(expression).length;
  if (commands > 1) throw generatedExpressionUnsafe(column.column, commands);
