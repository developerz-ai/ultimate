...
//
// A copy is not free here: the rule decides whether a row the caller never NAMED a column on is the
// same row as one that stored `null`, and the table holds NULL for both. `===` made them two, and
