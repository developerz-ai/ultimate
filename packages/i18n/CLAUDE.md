...
- Plural selection is `Intl.PluralRules`. Never `count === 1`. Variants are underscore suffixes
  on the leaf (`n_one`, `n_plural`), never a nested `{ one, other }` branch — the runtime probes
  `n_one`, and `TranslationKey` admits the stem `n` on exactly that basis.
