...
| `src/components/` | 58 components, `PascalCase.tsx` (component convention overrides the repo's kebab-case) |
...
| `src/barrel-bytes.test.ts` | the build error behind the two claims above: the setter's byte ceiling, and barrel-vs-deep-path parity — the second read off the RETAINED MODULE LIST (`minify: false` banners), never a byte allowance |
