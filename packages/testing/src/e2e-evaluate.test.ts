...
describe('e2e evaluate — what cannot cross', () => {
  ...
  // Row ab: the function-expression pattern required a NAME, so an anonymous `function (a)` read
  // as taking nothing and its parameter arrived `undefined` in the page.
  test('an ANONYMOUS function expression declaring a parameter is refused too', () => {
    const anonymous = new Function('return function (rows) { return rows; };')() as () => number;
    expect(anonymous.toString()).toContain('function (rows)');
    expect(() => closureSource(anonymous)).toThrow(/X_E2E_EVALUATE_UNSUPPORTED/);
    const none = new Function('return function () { return 1; };')() as () => number;
    expect(() => closureSource(none)).not.toThrow();
  });
