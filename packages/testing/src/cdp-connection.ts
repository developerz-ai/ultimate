...
export interface CdpConnection {
   ...
   *
   * The listener is also handed the event's `sessionId` (`undefined` for a browser-level event), so
   * a subscriber that owns ONE page can ignore the frames every other attached target sends.
   */
  on(method: string, listener: CdpEventListener): () => void;
  close(): void;
}

/** One event's params, and the session it arrived on. */
...
export async function cdpConnect(options: CdpConnectionOptions): Promise<CdpConnection> {
  ...
  await new Promise<void>((resolve, reject) => {
    // The socket is CLOSED on both failures: a handshake that timed out otherwise kept dialling
    // in the background, holding a descriptor and a pending connection for a caller that gave up.
...
export function cdpConnectOver(transport: CdpTransport, timeoutMs: number): CdpConnection {
  ...
  const message = (raw: string): void => {
    ...
    if (typeof id !== 'number') {
      ...
      const on = typeof frame['sessionId'] === 'string' ? frame['sessionId'] : undefined;
      for (const waiter of [...waiters]) waiter(method, on);
      ...
      if (subscribed !== undefined) {
        ...
        for (const listener of [...subscribed]) listener(payload, on);
      }
      return;
    }
