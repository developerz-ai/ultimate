...
// Row ac: `<input>` with no `type` IS a text box — the default type — and `getByRole('textbox')`
// did not list it, so the most common field on a form was invisible to the role query.
describe('e2e selection — an untyped input is a textbox', () => {
  test('input with no type attribute resolves as textbox, and a checkbox still does not', async () => {
    const form: FakeE2eElement = {
      tag: 'html',
      attrs: {},
      children: [
        { tag: 'input', attrs: {} },
    ...
    expect((await resolve(form, { kind: 'role', role: 'textbox', first: false })).count).toBe(1);
  });
});
