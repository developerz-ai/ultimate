...
describe('cdpConnect', () => {
  ...
  test('on hands every listener the session the event arrived on', async () => {
    install();
    const connection = await cdpConnect({ endpoint: 'ws://x/1', timeoutMs: 1000 });
    const seen: (string | undefined)[] = [];
    connection.on('Runtime.consoleAPICalled', (_params, sessionId) => seen.push(sessionId));

    socket().reply({ method: 'Runtime.consoleAPICalled', sessionId: 'mine' });
    socket().reply({ method: 'Runtime.consoleAPICalled' });

    expect(seen).toEqual(['mine', undefined]);
    connection.close();
  });
