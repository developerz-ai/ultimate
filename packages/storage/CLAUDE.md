...
- **The default is taken on `undefined` and on nothing else** — `x === undefined ? D : x`, never
  `x ?? D`: `??` defaults a blanked `null` before the screen can refuse it.

- **`MAX_KEY_LENGTH` is UTF-8 BYTES, measured in bytes** — S3's limit. A UTF-16 `length` passed
  400 CJK characters (1,200 bytes) that the store then refused. Same fix as `@ultimat3/cache`'s
  surrogate-key guard.
...
- **The local `list()` globs with `dot: true`, and the `META_DIR` skip is the real filter.**
  Without `dot`, dot-prefixed keys were missing from the listing while `put`/`get` handled them —
  and `sweepOrphans` pages through `list()`. Pinned in `driver-parity.test.ts`.
...
- **`VARIANT_FORMATS` is this package's format vocabulary, and `IMAGE_FORMATS` is core's** (what
  core can PROBE). `as const satisfies readonly ImageFormat[]` makes it a strict subset by the
  compiler; `isVariantFormat` takes `string` so `probeImage(bytes).format` narrows with no cast.
  `scripts/render-modes.ts` holds the `IMAGE_FORMATS` row and `image.test.ts` fails if either core
  name reappears in `src/index.ts`.
...
- **Bundle** (`bun build --target=browser --minify`, `uploadFile` only, `As of 2026-09-22`):
  **17,767 B** on `clientTransport`; the +5.9 kB over 20.2.1 is core's transport graph.
- `exactOptionalPropertyTypes` is on — declare optional fields as `x?: T | undefined`.

Why each rule above is shaped the way it is: [`docs/history/storage.md`](../../docs/history/storage.md).
