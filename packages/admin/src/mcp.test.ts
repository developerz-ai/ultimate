...
const GRANTS: Record<string, readonly string[]> = {
  reader: ['admin:read', 'admin_mcp_post:read'],
  editor: ['admin:write', 'admin_mcp_post:read', 'admin_mcp_post:write', 'admin_mcp_post:publish'],
};
...
describe('the admin MCP catalog is computed per caller', () => {
  test('a read-only actor is shown only the tools it may call', async () => {
    expect(await listTools(caller('reader'))).toEqual([
      'admin.admin_mcp_post.list',
      'admin.admin_mcp_post.read',
      'admin.search',
    ]);

    // The server really was built with the whole catalog — the caller was shown a subset of it.
    expect(mcp.tools.map((tool) => tool.name)).toContain('admin.admin_mcp_post.delete');
  ...
  test('a second caller against the SAME server gets a different catalog', async () => {
    ...
    expect(first).toContain('admin.admin_mcp_post.create');
    expect(first).toContain('admin.admin_mcp_post.update');
    expect(first).toContain('admin.action.post.publish');
    // Nothing implies `admin_mcp_post:delete`, so even the editor never sees the destructive tool.
    expect(first).not.toContain('admin.admin_mcp_post.delete');

    expect(second).not.toContain('admin.admin_mcp_post.create');
  ...
  test('the allowed-list cache is scoped to the caller OBJECT, which is the documented grain', async () => {
    ...
    GRANTS['promoted'] = ['admin:read', 'admin_mcp_post:read'];
  ...
  test('calling a tool the caller cannot see answers ToolNotFound, never Forbidden', async () => {
    const reader = caller('reader');
    const response = await callTool(reader, 'admin.admin_mcp_post.delete', {
      id: 'p_1',
      confirmation: 'admin_mcp_post:p_1',
    });
    ...
    expect(body).not.toContain('admin_mcp_post:delete');
    expect(body).not.toContain('admin.policy');
    expect(body).not.toContain('confirmation');
    expect(body).not.toContain('Delete one admin_mcp_post row');
  });
