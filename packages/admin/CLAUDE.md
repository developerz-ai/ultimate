...
- **The `/_x` SQL guard blanks every opaque span in ONE left-to-right pass** (`dev/panel-db.ts`): `'…'`, `"…"`, `$tag$…$tag$` and both comment forms in a single alternation, because each form can contain another's opener and no ordering of separate passes is correct. Blanking comments before quoted identifiers is what let `select 1 as "--"; delete from members` read as a plain SELECT while Postgres ran both statements. An unterminated quote matches nothing and leaves the rest visible — the guard fails closed, never open.
