...
| Not a deploy check | `update-available` answers a skew between the build the client claims — the `hello`'s `buildId`, or `?build=` on the dial; the hello's word is the later one and wins — and the node's own, and neither changes on an open socket, so every `hello` on one socket answers the same forever. A client hears about a deploy on the socket it opens against the **new** node |
