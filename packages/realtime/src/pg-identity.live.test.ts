...
const TABLES = [KEYED, KEYLESS, NOTHING, FULL];
...
describe.skipIf(!ready)('live · the replica identity warning', () => {
  ...
  test('names only the tables with no identity: no primary key, or NOTHING', async () => {
    ...
    warn.mockRestore();
    expect(line?.[1]).toMatchObject({ tables: [KEYLESS, NOTHING] });
  });
});
