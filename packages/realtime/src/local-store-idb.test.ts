...
import { fakeIndexedDb } from './idb-fake';
...
async function seed(store: LocalStore): Promise<void> {
  ...
  await store.saveQueue('p:u1', { mutations: [], nextSeq: 4 });
  await store.saveQueue('p:u2', { mutations: [], nextSeq: 7 });
}
...
describe.each([
  ...
  test('wipeOthers keeps ONE scope, rows and outbox, and takes every other', async () => {
    const store = await open();
    await seed(store);
    await store.saveQueue('anon', { mutations: [], nextSeq: 2 });
