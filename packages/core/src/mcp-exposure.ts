...
//
// Six readers across five packages decided it three ways until 2026-08: `=== true` where a tool is
// actually built, `!== false` in the OpenAPI hint and `?? true` in the manifest fact. So an action
// with no `mcp` block was published as a tool by the contract and refused by every surface that
// could have called one. Core owns the predicate because those readers span tiers 3-5 — `action`,
// `query`, `mcp`, `ai`, `manifest` — and this is the only tier all of them reach, the same reason
// `timing-safe-equal.ts` lives here.
