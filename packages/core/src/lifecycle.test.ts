// Single responsibility: the lifecycle state machine — starting -> ready -> draining -> stopped,
// the shutdown hooks each phase runs, and the readiness checks /readyz reports. The drain's time
// budget is `lifecycle-deadline.test.ts`; a logger that throws on the way is
// `lifecycle-logging.test.ts`.
