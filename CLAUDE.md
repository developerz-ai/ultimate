...
framework.manifest.json  GENERATED by `bun run manifest`: packages, tiers, every X_* code and
                its owner. Never hand-edited — drift fails `bun run verify` (X_MANIFEST_DRIFT)
```
