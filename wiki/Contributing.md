...
| a declaration key nothing reads | `bun run scripts/declaration-readers.ts` — a step of the gate's `unit` check, standalone. Every leaf key of every declaration an app writes (the type each primitive and each `define*` factory takes) needs a reader in `packages/*/src`, or a pinned reason. **173 leaves across 18 roots, ratchet at zero**, `As of 2026-08-24`. Written because `RouteBudget` declared `css`, `cls` and `tbt`, `registerRoute` flattened a budget to `budgetJs` + `budgetLcp`, and every reader downstream read only those two — so `budget: { cls: 0.1 }` was accepted, stored and ignored while `x verify`'s own `budgets` step reported green |
...
| Expectation | Detail |
|---|---|
| Green `x verify` | all 20 steps, in this order: typecheck, lint, boundaries, filesize, package-shape, errors, unit, contract, live, job, e2e, eval, drift, contract-diff, budgets, seo, i18n, policy, manifest, roadmap |
