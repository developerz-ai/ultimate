...
| `X_ACTION_RATE_LIMIT_INVALID` | an action's `rateLimit` cannot be run as a bucket | three checks, and the cause names which failed: `limit` finite and **≥ 1**, `windowMs` finite and > 0, and the **computed** `limit / (windowMs / 1000)` finite and > 0. `windowMs: 0` and `{ limit: MAX_VALUE, windowMs: 1 }` both compute an infinite refill — a bucket that never empties. Below one token the first caller is already refused, which is a policy's job and never a rate limit's | edit the `rateLimit:` to a whole allowance over a real window — `{ limit: 5, windowMs: 600_000 }` for five per ten minutes — or delete it to keep the `default` bucket |
...
| `X_OAUTH_TOKEN_INVALID` | the id token failed its issuer, audience or expiry check | the client id in `.env` is not the one the authorize URL was built with, or this host's clock is skewed | match `<PROVIDER>_CLIENT_ID` to the id `beginOAuth()` used, then restart at `GET /auth/oauth/<provider>` |
| `X_OAUTH_PROVIDER_UNKNOWN` | the URL named a provider this app has not enabled | the `:provider` segment is not a known provider, **or** is one but was left out of `defineAuth({ providers })`. One refusal for both: distinguishing them would describe the app's configuration to an anonymous caller | `add '<provider>' to defineAuth({ providers: [...] })` — the `fix` names the providers **Ultimate supports**, never the ones this app enabled, and `meta` carries only `{ provider }`: the enabled set is the configuration the shared refusal exists to keep from an anonymous caller, and a segment Ultimate has never heard of cannot be added to `providers` at all, so naming it left the fix unrunnable for the branch that matters |
...
| `X_MIGRATION_SNAPSHOT_MISSING` | the newest migration records no schema snapshot | `As of 2026-08`: `x db gen` found the newest migration on disk carrying no `<id>.snapshot.json` sidecar — deleted, or hand-written without one. Every generated migration writes the sidecar the *next* generation diffs against, so there is nothing to diff the entities against. Refused rather than defaulted to the empty schema, which would emit `create table` for every table the database already holds | restore the sidecar from version control, or delete that migration and regenerate it. The same condition reaches `x db migrate` as `X_DB_DRIFT` with an `unknown-schema` difference — the live catalog cannot be judged against a schema nobody wrote down |
...
slot costs nothing, it does not suppress the other — and otherwise to the error index; it never
synthesizes `https://ultimate.dev/errors/<code>` for a code no page here documents.
...
| `X_VERIFY_SUITE_VANISHED` | a step the committed floor requires had nothing left to check | `x.verify.json` names a step this repo already ran, and its suite was deleted — so the step reported as skipped and the gate would have stayed green | restore the suite, or delete that name from `x.verify.json` in the commit that says why — then `x verify --json` |
