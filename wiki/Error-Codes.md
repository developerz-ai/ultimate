...
| `X_ISLAND_NOT_HYDRATED` | a page renders an island nothing would boot | two cases, both deliberate now that a route carrying an island derives its own `hydrate`: the route states `hydrate: 'never'` **beside** an island — a contradiction the author wrote on purpose — or the `island()` call sits **below** the `defineRoute` that drains it, so the declaration had not happened yet | drop the explicit `hydrate: 'never'` and let it derive, or move the `island()` call above `defineRoute`; the cause says which of the two it is |
