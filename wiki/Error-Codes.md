...
| `X_CURSOR_SECRET_DEV` | cursors are signed with the shipped development key | `ULTIMATE_CURSOR_SECRET` is unset in a production environment, so the signing key is the constant published in `@ultimat3/core` and a client can forge a page position. Reported by `x doctor`, never thrown | set `ULTIMATE_CURSOR_SECRET` in the deploy environment: `openssl rand -hex 32` |
