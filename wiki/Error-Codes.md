...
| `X_RATE_LIMIT_BUCKET_CONFLICT` | a route and `app.config.ts` declare different numbers for one bucket | an action's `rateLimit: { limit, windowMs }` and an `http.rateLimit.buckets.<name>` entry disagree, or two routes claim one bucket name with different numbers. Refused at `createServer`/`createPipeline`, **before the socket opens** — picking a winner would leave the loser a number someone reads and nothing applies, which is the defect this check exists to close. An identical restatement passes | delete `http.rateLimit.buckets.<name>` from `app.config.ts`, or drop the route's `rateLimit` — the cause names both number pairs and which side is the config |
...
| `X_ACTION_RATE_LIMIT_INVALID` | an action's `rateLimit` half is not a usable number | a non-positive or non-finite `limit` or `windowMs`. `windowMs: 0` is an infinite refill — a declared limit that enforces nothing — so it is refused rather than published to a spec the limiter cannot run on | make both positive: `rateLimit: { limit: 5, windowMs: 600_000 }`, or delete the block and stay on the `default` bucket |
