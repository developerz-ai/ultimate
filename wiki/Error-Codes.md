...
| `X_AI_REQUEST_INVALID` | the provider would reject this request | a reasoning control the chosen model does not have — `effort` or adaptive `thinking` on a pre-4.6 model, or `thinking: 'disabled'` above a model's effort cap | set `model:` to one that has the control, or drop the control from the prompt |
...
| `X_LLM_REFUSED` | the model declined the request | the provider's safety classifiers refused; `stopDetails.category` names why | rephrase the prompt template and bump its version, or set `model:` on the `llm()` declaration to one without that classifier |
| `X_LLM_TRUNCATED` | the answer hit its `maxTokens` ceiling before it was complete | the ceiling is below what the output schema asks for | raise `maxTokens` on the `llm()` declaration, or ask the schema for less |
