...
| `X_STORAGE_NOT_FOUND` | no object at that key | a stale key, the wrong disk, or an object already deleted | list the prefix and compare: `await disk('<disk>').list({ prefix: '<dir>' })`. The thrown `fix` names `x storage ls <disk> --prefix <dir> --json`, which is not a shipped command yet |
...
| `X_TEST_SHARD_FAILED` | a test shard exited non-zero | a red test inside one worker of a sharded run | the `fix` replays that shard alone: `x test <type> --workers <n> --worker <i>` |
