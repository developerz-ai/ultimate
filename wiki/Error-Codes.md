...
| `X_ISLAND_NOT_HYDRATED` | a page renders an island nothing would boot | the route declares `hydrate: 'never'`, or the render supplied no island collector — either way the markup is inert and the interactive part silently does nothing | set `hydrate` to `idle`, `visible` or `interaction` on the route, or remove the island |
