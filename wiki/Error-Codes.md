...
| `X_LIVE_QUERY_UNKNOWN` | no live query is registered under the name a `subscribe` frame asked for | a misspelled name, or a query never passed to `defineApi({ queries })` — the frame parsed and the version matched, so this is not a protocol skew and a client rebuild cannot change it | `x queries list --json` — subscribe under a name it prints, and register the query if it is missing |
