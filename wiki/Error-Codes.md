...
| `X_TENANCY_UNSCOPED` | a tenant-scoped query has no org predicate | a repo call outside any request context, so there is no actor to derive the tenant from; or an `upsertAll(rows, { onMatch: 'update' })` whose `onConflict` omits the tenant column — a collision there lands on another tenant's row | run inside the request context, so the actor scopes the plan; from a script, `runWithContext(createContext({ actor }), fn)`; or put the tenant column in `onConflict` |
...
| `X_TENANCY_CROSS_DENIED` | a cross-tenant read was entered without the capability | `crossTenant()` called by an actor lacking `tenancy:cross`, or outside every request context | grant it where the actor is minted — `serviceActor({ id: 'reconciler', scopes: ['tenancy:cross'] })` — and run inside `runWithContext(createContext({ actor }), fn)` |
