...
| `X_REFERENCE_APP_PIN_STALE` | a step pinned as failing in the app's `expectedRed` now passes | the app was repaired and the pin was not lowered — the ratchet only shrinks | delete the named entries from that app's `expectedRed` in `scripts/lib/gated-apps.ts` |
