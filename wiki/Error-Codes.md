...
| `X_OAUTH_PROVIDER_UNKNOWN` | the URL named a provider this app has not enabled | the `:provider` segment is not a known provider, **or** is one but was left out of `defineAuth({ providers })`. One refusal for both: distinguishing them would describe the app's configuration to an anonymous caller | `add '<provider>' to defineAuth({ providers: [...] })` — the `fix` names the currently enabled set; `meta` carries `{ provider, enabled }` |
...
| `X_TEST_REGISTRY_LEAK` | a test file left a process-global registry dirty | a `declareTags()` or `registerTier()` call inside a test body with nothing to undo it. Boot calls are the file's to clean up, and a leaked one silently changes the *next* file's premise — `assertKnownTags` short-circuits while nothing is declared, so one leaking file switched tag validation on for the rest of the process and a different package's tests failed. The `cause` names every offending file and what each left behind, never the file that happened to be running when it surfaced | the `fix` names the file: `const restore = isolateDeclaredTags(); afterAll(restore)` for tags, `afterAll(resetTiers)` for tiers. Entity and job registries are **not** covered — declaring at module scope is the idiom there, so a test that assumes an empty one calls `isolateEntityRegistry()` instead |
