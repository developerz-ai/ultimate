...
v1.0.0 `As of 2026-08`. Stable API — semver from here ([Upgrading](Upgrading)). `x build --target docker|binary|static`, the compose files, and the Helm chart all ship; what is still outstanding in milestone 11 ([roadmap](https://github.com/developerz-ai/ultimate/blob/main/docs/idea/14-roadmap.md)) is the two-platform proof — the demo app on Compose **and** on K8s from one image, with a rolling restart invisible to connected clients.
