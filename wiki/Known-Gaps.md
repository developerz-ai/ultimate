...
| `recordJob` | declared, still uncalled — `jobs_total` is always empty | the other three series are live |
| `.env.example` drift | `assertEnvExample()` exists and no verify step, CLI command or lifecycle hook calls it | call it in your own test → [Configuration](Configuration) |
...
| Generated entities fail `typecheck` | not only `x new`'s example slice — `x g resource` and `x g entity` emit the same `invariant()` calls, so **every** generated entity is red until you add `!` | add the non-null assertion the compiler asks for |
...
| `X_MIGRATE_CONCURRENT` | reserved, not thrown — **but the lock is real**. `migrate()` wraps every run in `pg_advisory_lock(MIGRATION_LOCK_KEY)` unless `lock: false` (only `x db branch` passes it), and `ROLE=migrate` does not. Two overlapping deploys serialise: the second **waits**, it does not race and does not error, which is why the code has no throw site. No pipeline serialisation needed |
