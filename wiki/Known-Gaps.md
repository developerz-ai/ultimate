...
| Foreign keys in drift | neither half is compared: `snapshotOf()` records `foreignKeys: []`, so a constraint dropped on the database by hand is invisible to `x db migrate`'s check. **Not in `CHANGELOG.md`'s list** | read them off the catalog yourself, or re-run the migration that declared them → [Entities and migrations](Entities-And-Migrations) |
