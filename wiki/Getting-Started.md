...
v1.1.0 `As of 2026-08`. Stable API — semver from here ([Upgrading](Upgrading)). 27 `@ultimat3/*` packages plus the unscoped `create-ultimate` — 28 in all — publish in lockstep. Milestones 0–10 are ✅; milestone 11 is 🚧, open on its two-platform deploy proof. Realtime tiers 1–2 are v1, tier 3 (local-first) is v2. The 50k-socket forced-restart benchmark **is measured and committed** — p50 54.0s / p90 105.5s time-to-consistent, on one node ([Realtime](Realtime)). Status markers come from [`docs/idea/14-roadmap.md`](https://github.com/developerz-ai/ultimate/blob/main/docs/idea/14-roadmap.md). See [FAQ](FAQ).
