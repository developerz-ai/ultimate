...
| `app/` / `api/` | resolution order `query → cookie → user → header`, configurable via `configureLocales({ order })`. The `user` rung is the actor's saved `locale`, which your `authenticate` hook sets (`userActor({ id, locale, tz })`); `@ultimat3/http` re-resolves once `auth` has run. **Until 21.0.0 that rung was never filled**: the `locale` stage ran before `auth` and read only the cookie and the header. An explicit choice outranks the browser: `?locale=` is how an email preview link pins a locale, the cookie is what the language switcher writes, and `Accept-Language` is only the fallback when nobody has chosen |
