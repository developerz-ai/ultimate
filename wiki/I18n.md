...
| Arity drift | a placeholder in one locale that no other locale has is reported by `x i18n check` |
...
| Command | Does |
|---|---|
| `x i18n add <locale>` | create `catalogs/<locale>.json` seeded with every `en` key, values empty |
| `x i18n sync <locale>` | add keys present in source but missing from that locale; never deletes |
| `x i18n check --json` | extract keys from source, diff against every catalog, report missing / unused / arity drift |
| `x i18n extract --json` | key usage report with file + line per key, including dynamic-key call sites |
