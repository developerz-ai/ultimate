...
| 5 | **Forced reload after a grace period** | `updatePolicy({ graceMs, forceOn })` — `graceMs` defaults to `DEFAULT_GRACE_MS`, **6h**, and `forceOn` to `['security']`. `updateSignal()` sets `forced` once a stale client passes the grace, and `deadlineAt` is **`now`**, not a countdown the framework runs. Your app decides what to render and when to reload. There is **no** deploy flag for this `As of 2026-08-20`: `x deploy --critical` was deleted in 4.0.0, having been echoed into the deploy plan and read by nothing |
