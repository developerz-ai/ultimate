...
| `x verify --json` | the gate — 20 steps, in this order: typecheck, lint, boundaries, filesize, package-shape, errors, unit, contract, live, job, e2e, eval, drift, contract-diff, budgets, seo, i18n, policy, manifest, roadmap |
...
| `ROLE=migrate` sits there doing nothing | another version's `ROLE=migrate` holds the session-pinned migration lock. This one **waits up to 60s**, polling once per 500ms, then exits non-zero with `X_MIGRATE_CONCURRENT` (`As of 2026-08`; on 1.2.0 it waits forever) | let the first exit 0 — the second then applies. If nothing else is deploying, the holder is a wedged backend: `psql "$DATABASE_URL" -c "select pid, application_name, state from pg_stat_activity join pg_locks using (pid) where locktype = 'advisory'"`, then `pg_terminate_backend(pid)` it |
...
| A client is connected but receives nothing, and never reconnects | half-open socket: the TCP connection died without a `close`, so no handler fired | the page socket's heartbeat ends it: 15 s, two silent windows, then close `4000`. In 21.0.0 there is no option to turn it off → [Realtime](Realtime) |
| Topics silent after the first reconnect, live queries fine | a client that does not re-announce its topics. **`hello` carries nothing to resume with** `As of 2026-08` — no cursors, no membership; a `subscribe` frame carries both, one per topic and one per live query with that subscription's cursor | the page socket re-sends all of them on open. A hand-written client must send one `subscribe` per topic **and** one per live query carrying its resume cursor, or the topics are silent, presence sweeps the membership, and every live query re-snapshots → [Realtime](Realtime) |
...
| `X_SW_HAND_EDITED` | **reserved, never raised** — `x build` writes `sw.js`, `As of 2026-08`, but it carries no checksum, so an edit survives a build ([Error codes → Not thrown yet](Error-Codes#not-thrown-yet)) | change the **route**, which is the input the generator is designed around; the emitted file is overwritten on the next build either way |
...
| Precache budget warning | `x build --target static --json` reports it under `serviceWorkerWarnings`, `As of 2026-08` — a manifest past 5 MB stalls the install on a bad connection. `PrecacheManifest.warnings` was computed and read by nothing until then ([#390](https://github.com/developerz-ai/ultimate/issues/390)) | drop routes from `offline: 'precache'`, or cut the island chunks the manifest carries — `x build --target static --json` lists both |
| Offline shows the browser dinosaur | `pwa.enabled` is false, or the page was never controlled — a worker that has activated is not yet the page's controller, and a first load is not controlled until it claims | check `navigator.serviceWorker.controller !== null` in the console; then → [PWA and offline](PWA-And-Offline) |
