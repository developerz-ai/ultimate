...
| Topics silent after the first reconnect, live queries fine | a client that does not re-announce its topics. `hello` carries live-query cursors only; topic membership is state on the node's socket | `LiveClient` re-sends every topic on open `As of 2026-08`. A hand-written client must do the same, or its presence membership is swept too |
