...
| Need everyone off a build now | security patch | your app calls `updateSignal({ ..., reason: 'security' })` from `@ultimat3/pwa` and reloads on `forced: true` — past `graceMs` (default **6h**) the signal carries `deadlineAt: now`. There is no deploy flag that sets this — `x deploy --critical` was deleted in 4.0.0, having been echoed into the deploy plan and read by nothing |
