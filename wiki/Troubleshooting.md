...
| Precache budget warning | `x build --target static --json` reports it under `precacheWarnings`, `As of 2026-08-27` — a manifest past 5 MB stalls the install on a bad connection. `PrecacheManifest.warnings` was computed and read by nothing until then ([#390](https://github.com/developerz-ai/ultimate/issues/390)) | drop routes from `offline: 'precache'`, or cut the island chunks the manifest carries — `x build --target static --json` lists both |
