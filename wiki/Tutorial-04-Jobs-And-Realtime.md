...
The Redis and NATS **job** drivers are deferred to v2 — each throws `X_NOT_IMPLEMENTED` behind an interface that already ships, rather than pretending to work. Postgres is the v1 driver, and it is the one `x dev` boots.
...
**Delivery.** A second run, 10,000 clients and a probe every 200ms, counting holes in each client's sequence: **1,666,882 channel patches received, 0 lost** ([`10k-restart-seq.json`](https://github.com/developerz-ai/ultimate/blob/main/scripts/bench/results/10k-restart-seq.json)). A first-patch timer cannot see a lost patch, which is why the second run exists — `As of 2026-08` it is the only one with delivery accounting, and it says nothing about 50,000.
