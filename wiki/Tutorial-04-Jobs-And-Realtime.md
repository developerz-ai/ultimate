...
| one leader | leadership is an expiring lease row in `x_scheduler_leader`, never an advisory lock — that grant is session-scoped and dies when a pooled connection returns. A second `scheduler` is a warm standby, not a duplicate |
...
The Redis and NATS **job** drivers are not in 4.0.0 — each throws `X_NOT_IMPLEMENTED` behind an interface that already ships, rather than pretending to work. Postgres is the shipped driver, and it is the one `x dev` boots.
...
Realtime tier 3 (local-first, `persist: true`) is not in 4.0.0.
