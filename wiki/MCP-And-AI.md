...
| executes code | `tests.run`, `verify.run`, `ui.shot`, `ui.island`, `ui.inspect` | scope `dev:test`; all five declare `destructive: true`, so none is metered as read chatter — the three `ui.*` tools launch a browser |
