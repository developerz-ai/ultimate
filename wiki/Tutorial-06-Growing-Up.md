...
| job queue | `@ultimat3/jobs` · `JobDriver` | `jobs.driver: 'postgres'`. Redis and NATS drivers are interface-complete stubs that throw `X_NOT_IMPLEMENTED` — not in 2.0.0 |
...
| realtime tier 3 (local-first, `persist: true`), the plugin API, multi-region replication | not in 2.0.0 |
