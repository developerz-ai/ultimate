...
| the Dockerfile `x new` writes you | **not the same image.** `oven/bun:1.3-alpine`, `ENTRYPOINT ["bun", "apps/web/server.ts"]`, user `bun`, ~194MB — measured on the `1.4-alpine` base and NOT re-measured since the base moved back to `1.3-alpine` on 2026-08-27, so read it off your own image rather than quoting it. Harden against *this* one — the uid and the shell differ, so the `runAsUser: 65532` baseline below does not transfer unchanged |
