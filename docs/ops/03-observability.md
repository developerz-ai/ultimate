...
| `/healthz` and `/readyz`, both returning a JSON body with named checks | **shipped** — `registerReadinessCheck(name, check)`, and `/readyz` is ready only when the state is ready **and** every check passes — and `every` over an EMPTY registry is vacuously true, so a process that registered no check answers **200** while checking nothing. `HealthReport.registered` is the field that tells those apart; read it, not just `ok`. Today the boot registers `database`, and `transport` where the transport can be disconnected ([`packages/core/src/lifecycle.ts`](../../packages/core/src/lifecycle.ts)) | works today: probes and blackbox checks. **`ReadinessCheck` is `() => boolean` — synchronous by design.** A probe that awaits a dependency turns a slow dependency into a wedged endpoint and then a restart loop, so whoever owns the dependency keeps a boolean fresh and the check just reads it. `/healthz` deliberately ignores the checks: a database outage that failed liveness everywhere would restart the whole fleet |
