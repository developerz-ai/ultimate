...
| 7 | `updateSignal` (`pwa/version-skew.ts:171`) | no runtime caller; `cmd-deploy.ts:150` names it as the wire behind `x deploy --critical` — a security-shaped flag that currently does nothing | Wire it in the same PR that makes `--critical` real, or delete both the export and the flag. Not this plan's call |
