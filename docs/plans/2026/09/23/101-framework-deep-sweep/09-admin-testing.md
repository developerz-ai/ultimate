...
|---|---|---|---|---|
| a | `args.confirmation !== args.expectedConfirmation` is `false` when both are omitted (`undefined !== undefined`), so a destructive action runs unconfirmed through the public `invokeAdminAction` | `packages/admin/src/action-gate.ts:137`, `index.ts:13` | Derive the expected token in the gate from `confirmationToken(action.entity ?? 'admin', args.subject?.id ?? '')` (`crud.ts:320`), and ignore a caller-passed `expectedConfirmation`. The token is a type-to-confirm guard against **accidents** (the same design as `adminDestroy`, `crud.ts:305-317`), not a secret. A hostile caller can compute it, and what stops that caller is the `admin:destroy` permission check the gate runs first. State this in the `InvokeInput.confirmation` doc comment (`action-gate.ts:97`) | patch (keeping the input field; removing it is in 18) |
...
2. b: failing test first. `adminDestroy`, `adminUpdate` and `adminCreate` on a resource declared `operations: ['list','detail']` each refuse, and the row still exists.
3. d: register the three codes with `bun run new-error-code … --package testing`.
...
- Direct CRUD on an operation the resource does not declare is refused.
