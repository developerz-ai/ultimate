...
| b | `adminCreate`, `adminUpdate` and `adminDestroy` never check `resource.operations`, so a direct call deletes on a `['list','detail']` resource | `packages/admin/src/crud.ts:221,253,306` | Refuse when `!resource.operations.includes(op)`, with the same code `canOperate` answers | patch |
