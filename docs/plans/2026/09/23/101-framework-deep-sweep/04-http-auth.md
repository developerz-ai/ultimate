...
| l | **production 500 bodies carry the Postgres message and the SQL statement.** `isUnclassifiedFailure` hides the cause only for codes with no status row, and `X_DB_STATEMENT_FAILED` has one (500) | `packages/http/src/error-facts.ts:294-295`; `packages/db/src/errors.ts:241-250` | Hide the cause and detail for every 5xx unless the code opts in via `registerProblemMeta({ publicCause: true })`. Initial opt-ins: `X_DRAINING`, `X_OVERLOADED`, `X_FLIGHT_GATE_OVERLOADED`, `X_TIMEOUT` | patch |
