...
| k | `mutator()` drops `row`, `rateLimit` and `deprecated`, so a row-level policy on a mutator gets `row === null` and denies every call. `README.md:218` says a mutator IS an action; the social clone works around this (`notifications/policy.ts:4-10`) | `packages/action/src/mutator.ts:74-96,134-143` | Add the three fields to `MutatorDef` and spread them into `actionDef` | minor |
| l | `x jobs retry` on a **running** job leaves `claimed_by` set and state `ready`, so a second worker claims it and it runs twice. Requeueing a `done` job whose key a live job holds raises raw 23505 on pg; memory accepts it and ends with two live rows | `packages/jobs/src/driver-pg-jobs-sql.ts:191-196`, `driver-memory.ts:140`, `driver.ts:178` | `where id = $1 and state in ('dead','cancelled','done')`. A live row is refused with `X_JOB_NOT_REQUEUEABLE`, fix `x jobs cancel <id> first`. Check the key first and answer `X_JOB_DUPLICATE` (exists) | patch |
| m | `QueueStats` counts a due delayed job as `delayed`, never `ready`, and `oldestReadyMs` skips it, so the autoscaler (`queue_depth`, `worker.ts:110`) cannot see that backlog | `packages/jobs/src/driver-pg-sql.ts` `SQL_STATS`, `driver-memory.ts:309-312` | Ready = `state in ('ready','delayed') and run_at <= now()`, aged by `run_at`. Delayed = `run_at > now()`. Same in memory | patch |
| n | `requeue(fromStep)` drops only the named step, although `driver.ts:178` says "from that step onward". Later steps replay results computed from the old run | `packages/jobs/src/driver-pg.ts:216-223`, `driver-memory.ts:136-138`, `wiki/CLI-Reference.md:864` | Recommended: delete steps with `started_at >=` the target's in both drivers, keeping the documented meaning. Record the decision in `status.yml` | patch |
