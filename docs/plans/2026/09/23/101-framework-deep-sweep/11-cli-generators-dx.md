...
| l | an `async Page` with no `load` passes the gate. The social clone has 11 of them (slice 16) | `packages/render/src/registry.ts` / `x verify` routes check | New finding `X_ROUTE_ASYNC_PAGE`, cause `<file> exports an async Page, so its data read bypasses load's caching and meta`, fix (an edit, which the CLI error contract accepts) `in <file>: move each await in Page into export const load = (ctx) => …, and read it with the page's data prop` | new gate finding | minor |
