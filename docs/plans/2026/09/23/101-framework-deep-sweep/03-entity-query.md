...
| d | `decimal()`/`bigint()` `$parse` does not canonicalise, so memory returns `'007.5'` where Postgres returns `7.50`, and `sum`/`max` differ | `packages/entity/src/columns-data.ts:48,73,123-152` | Canonicalise **accepted** input only, in the parse function after its existing refusals: strip leading zeros, `-0`/`-0.00` → `0`/`0.00`, pad a short fraction to the declared scale. Excess scale and integer overflow stay **refused**, as `columns-data.ts:137-150` already does (`refuseColumn`). Never round: rounding would widen the input contract. Decimal-string work only, never `Number`. Parity cases: `'007.5'`, `'2.5'`, `'-0'`, plus the existing refusals `'1.234'` at scale 2 and overflow | patch |
...
| k | memory accepts what Postgres refuses: `text({max:3})` with `'abcdef'` (pg 23514), `integer()` with 3e9 (pg 22003), `url()` with `'HTTPS://a.b'` (a case-sensitive CHECK). A non-integer or NaN `max` emits `<= NaN` into the DDL | `packages/entity/src/columns.ts:106,110-118,152-171` | Enforce each in `$parse`: code-point length ≤ max, the int4 range, and a scheme check matching the CHECK (make the CHECK case-insensitive, `~*`). Refuse a non-integer `max` at declaration | patch |
| l | `timestamp()` parses a zoneless date-time in the host TZ on insert and seed (`query.ts:441`, `seed.ts:270,286`) | `packages/entity/src/columns.ts:131-142`; `schema/src/iso-date.ts:21` (`isZonelessDateTime`, not exported) | Export the predicate from schema and refuse zoneless strings in `parseInstant` | **major** with 18 i |
