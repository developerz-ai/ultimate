...
| d | `decimal()`/`bigint()` `$parse` does not canonicalise, so memory returns `'007.5'` where Postgres returns `7.50`, and `sum`/`max` differ | `packages/entity/src/columns-data.ts:48,73,123-152` | Canonicalise in parse: strip leading zeros, `-0` → `0`, pad the fraction to the declared scale | patch |
...
| f | `min`/`max` on `timestamptz` goes through a bare `::text`, then `new Date(text)`. That gives NaN for an offset with seconds, and year `0099` becomes 1999 | `packages/entity/src/aggregate-decode.ts:26-28`, `pg-sql.ts:443` | `(… at time zone 'UTC')::text` (the `seekPrecision` approach), or epoch ms | patch |
