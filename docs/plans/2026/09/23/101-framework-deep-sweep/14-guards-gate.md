...
| # | Change | File:line |
|---|---|---|
| a | `bun run verify --only <step>` silently runs all 20 steps. Forward `--only` (validated against `VERIFY_STEP_NAMES`) and refuse unknown flags with `X_CLI_BAD_FLAG` (cause `unknown flag --<x>` or `unknown step <x>`, fix `bun run verify --only <nearest of VERIFY_STEP_NAMES>`) | `scripts/verify.ts:305-310` |
| b | `scripts/lib/corpus.ts`: named scopes (`shipped` = `packages/*/src` minus tests, `tests`, `apps`, `scripts`, `docs`), each read and masked once per process with `packages/core/src/source-mask.ts`, cached. Each scope has a **floor** (e.g. `shipped ≥ 1000` files) and refuses `X_CORPUS_UNSCANNED` below it (cause `scope <s> read <n> files, floor <m>`, fix `bun install && bun run <guard>`: the usual cause is an unreadable or half-checked-out tree). Every guard moves onto it. Delete the 7 spellings of `'packages/*/src/**/*.{ts,tsx}'` (`flight-copies.ts:20`, `frozen-records.ts:273`, `render-modes.ts:318`, `config-readers.ts:76`, `declaration-readers.ts:64`, `lib/i18n-scan.ts:15`, `lib/browser-barrel-set.ts:25,126`) | new `scripts/lib/corpus.ts`; `scripts/boundaries.ts:272` `collectSourceFiles` |
...
| g | **`pin-raises`**: a pin row higher than at `origin/main`'s tip needs `why:` on the row, else `X_PIN_RAISE_UNSTATED` (cause `<pins file> <row> rose <old>→<new> with no why:`, fix `add // why: <reason> on that row, or fix the new sites and bun run <guard> --unpin <pkg>`). It has the same shape and fetch as `budget-raises`. History shows silent raises: secret-compare 53 → 63, `node-import-pins` 3→12, `proto-index-pins` 1→9 | new `scripts/pin-raises.ts`, wired as a `unit` step check |
...
| i | `budget-raises` compares nothing without `origin/main`, including on a tag checkout in `release.yml`. When the ref is missing, fetch it (`git fetch --no-tags --depth=1 origin main`) or refuse `X_BUDGET_BASE_MISSING` (fix `git fetch --no-tags --depth=1 origin main`), never pass | `scripts/budget-raises.ts` |
...
| o | `llms.txt` claims to be generated and is not (notify is missing). Write a generator from `scripts/list-workspaces.ts` and the wiki sidebar, plus a drift check `X_LLMS_TXT_DRIFT` (cause names the missing or extra entry, fix `bun run llms-txt --write`) | new `scripts/llms-txt.ts`; `llms.txt` |
| p | `CLAUDE.md` byte ceiling: refuse root `CLAUDE.md` above 16 KB, and a package `CLAUDE.md` above 24 KB (a ratchet whose pins are each file's size **after** 17 f lands, so it refuses only later growth. Enable it in the same PR as 17 f, never before) | new rule, paired with slice 17 f |
...
| r | **root cause of #450.** `tsc -b` decides "up to date" by mtime, and Bun installs packages as hardlinks from its cache carrying the cache's old mtimes. An upgrade to an already-cached version is then skipped and reports green locally while CI (cold) is red. When the root tsconfig has no `references` (every scaffolded app), run `tsc -p . --pretty false` (incremental, content-hashed). Keep `-b` for the framework root. Test: back-date an input and expect red. Close #450 | `packages/cli/src/verify-checks.ts:61`, `templates/scaffold-repo.ts:122-147` |
...
5. n and o alongside slice 17. p lands in the same PR as 17 f.
