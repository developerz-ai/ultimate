...
| j | **milestone 11 proof**: nothing in CI builds an app image, installs the chart, or measures a rollout | `.github/workflows/ci.yml` (new job `deploy-proof`) | kind or k3d on `ubuntu-latest`: build the scaffold image, `helm install`, `helm upgrade` with a changed `BUILD_ID`, while a load loop sends mixed GET and POST. Assert 0 non-2xx. Budget ≤ 8 min, main-only (a `paths` filter on `.github/workflows/ci.yml`, `docker/**`, `packages/cli/src/templates/scaffold-*`, `packages/core/src/lifecycle.ts`, `packages/http/src/server.ts`) |
