# 17 — Docs and the major

> Part of [`overview.md`](overview.md). Depends on: 01–16. Tier: docs.
...
- `wiki/Error-Codes.md` — every code from slices 01–15.
- Root `CLAUDE.md` realtime section — delete "unrepairable, not uncounted" once slice 10 lands, restate with the new bench result.
...
- `CHANGELOG.md` `[Unreleased]` + `wiki/Upgrading.md` — one `BREAKING —` entry per removed surface, each naming its manual edit: `setLiveClient`/`connect()` in islands, string `subscribe(topic)`, client `publish`, `ClientSocket` injection, OPFS store, `/_x/outbox/flush`, `IdentityMap` rename, `useLive`, `liveHookFor`, `onMutate` + `mutate`/`ack`/`rebase` frames (sync protocol bump), `custom(merge)` over rows, `AsyncState` import path. `bun run changelog-check`.
