# 05 — Typed channels that write the store

> Part of [`overview.md`](overview.md). Depends on: 04. Tier: 3.

## Files to change
- `packages/realtime/src/channel.ts` — `channel(name, { params, records?, events? })` declaration: `name` + a typed param builder is the ONLY way to spell a topic string. Server publish and client subscribe both take the declaration, never a string.
...
3. Delete `publish(topic)` on the client (`client.ts:291` sends a `patch` frame from the browser): client writes go through mutators only (axiom 1).
...
- Guard `channel-literals` (slice 08) green at zero.
