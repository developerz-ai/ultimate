...
- **Bundle bytes.** `rpc` 14.8 kB, `queryClient` 12.8 kB; value-importing `LiveClient` took a chunk 8.4→26.6 kB. The transport stays value-light; the socket module loads via `import()`. If an island budget still breaks, first cut bytes that buy no function; bytes that buy real function raise the budget by the measured amount, number and reason in the same diff (axiom 9, decided 2026-09-22). Never reintroduce raw `fetch`.
