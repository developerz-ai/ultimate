# 04 — RecordStore and one socket per page

> Part of [`overview.md`](overview.md). Depends on: 01, 02. Tier: 3.

## Files to change
- `packages/realtime/src/identity-map.ts:24,35` → `record-store.ts` (rename; delete the old file) — key is ALWAYS `recordType:recordKey` from slice 02; `privateScope(query)` deleted. Keeps retain/release + batched notify. Implements core's `RecordSink` (`adopt`, `remove`). Rows validated by the entity schema on adopt; a row failing it → `X_RECORD_REJECTED` (realtime `errors.ts`), dropped, logged — never partially merged.
...
- `packages/realtime/src/live-rows.ts:31` — live query windows hold ordered ids only; rows come from the store. Same for plain `query.client()` list results adopted in slice 03.
...
- `packages/realtime/src/hooks.ts:23,26` — module singleton `setLiveClient` deleted; hooks read `pageClient()` (core, slice 01). New `useRecord(entity, id)` → accessor of the one record; `useRecords(entity, ids)`. `useLive` returns ids + `useRecord`-backed rows.
