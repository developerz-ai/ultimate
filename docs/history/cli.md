# @ultimat3/cli — history

The reasoning moved out of [`packages/cli/CLAUDE.md`](../../packages/cli/CLAUDE.md) (plan 101,
...
`runtime-replica.ts` is where read-replica routing is WIRED, and it had to be wired in two places
...
calls `setDbClient` in `runtime-queue.ts`, so no booted process had ever read that variable. Routing
...
`role-sync.ts`'s handed back the port the finding had just named as taken, each under a test that
...
| | Reaches for | Never |
|---|---|---|
| `x shot <route>` | `x dev` on a scratch port, plus Chrome over raw CDP (`cdp-shot-driver.ts`, no browser library anywhere) — launched here on its debugging pipe by `@ultimat3/testing`'s `launchChrome`, or **attached** to over `--cdp-url` / `SCRAPE_CDP_URL`, which is what every stealth provider sells | the static build — `--target static` prerenders `site/` only, so an `app/` route would photograph the landing page |
...
### `x shot`'s browser is raw CDP, and its port is this package's

| File | Job |
|---|---|
| `browser-launcher-port.ts` | `ShotDriver` / `ShotSession` / `ShotPage` — every member a shot or `ui.*` reads, nothing else |
| `browser-launcher.ts` | which browser a run gets, and `appBrowser`, refused before any boot |
| `cdp-shot-driver.ts` | one `open()` is one launched or attached browser and one page |
| `cdp-shot-page.ts` | the verbs; a click is a real pointer press, a chord real key events |
| `cdp-shot-watch.ts` | bounded console / error / network tails; the allow list enforced in the browser |
| `cdp-shot-element.ts` / `cdp-shot-keys.ts` / `cdp-shot-a11y.ts` | readiness, key chords, the accessibility tree |
| `browser-launcher-fake.ts` | `fakeShotDriver`: recorded pages, queried with `HTMLRewriter`, for offline tests |

`e2e/cdp-shot.e2e.test.ts` drives each verb in Chrome.
...
`assertIslandStatesPure`. `cli → testing` is a declared sideways edge, and it carries the
screenshot half too: `x shot` launches on testing's `launchChrome` (22.0.0).
...
## The browser-backed e2e driver moved to `@ultimat3/testing` (22.0.0)

`e2e-*` and `cdp-*` live in `packages/testing/src` now; its `CLAUDE.md` carries what this section
said. The raw-CDP browser won (plan 101, 18 e): no `@ultimat3/scraping` and no `puppeteer-core`
under the e2e step, `x shot` or `mcp-ui*` (22.0.0).
...
**The filename says where a module runs**, `As of 2026-09-23`: `role-*` and `runtime-*` are what
`serve.ts` boots in production as well as under `x dev`; `dev-*` is `x dev`'s alone (the watcher,
the reload, the `/_x` dashboard sources, the dev lock). They were all `dev-*` until plan 101 slice
12 c, so "what does the container run?" had no answer short of walking `serve.ts`'s imports.

| File | Job |
|---|---|
| `api-routes.ts` | the app's API over HTTP: every registered action AND every registered query |
| `runtime-bindings.ts` | resolve which service each binding points at — embedded or external |
| `runtime-queue.ts` | the db + queue pair alone, and the one place that takes every ambient accessor back |
| `runtime-services.ts` | start the rest on top of it and install the remaining accessors (storage, mail, transport) |
| `runtime-cache.ts` | which cache tiers this process reads through, and the cross-instance invalidation hop |
| `runtime-purge.ts` | the hourly retention sweep: which framework tables this boot owns, the `purge()` job over them and the `task` that fires it |
| `runtime-notify-retention.ts` | `notify.inboxReadRetentionMs` / `inboxUnreadRetentionMs` off the app's own `app.config.ts` — the sibling of `loadSignInPath` and `loadCacheTiers`, because `startServices` holds no `AppConfig` |
| `role-sync.ts` | the `sync` role: its live-query registry, who is dialling it, and the socket it owns |
| `runtime-realtime.ts` | `realtime.enabled` / `transport` / `urlEnv` off the app's own `app.config.ts`, for the boot — `@ultimat3/realtime`'s `selectTransport` then dials the variable `urlEnv` names |
| `role-realtime.ts` | `realtime.enabled`, obeyed where the roles are chosen: off means no `sync` node and no `replicator` |
| `runtime-jobs.ts` | `jobs.queues` / `concurrency` / `visibilityTimeoutMs` off the app's own `app.config.ts`, for the `worker` role's `createWorker` |
...
| `runtime-render.ts` | one HTTP route per registered `route`, through render's own mode function |
...
| `runtime-assets.ts` | the image pipeline's only HTTP surface: `/icons/*` and `/media/*` |
| `favicon.ts` | `/favicon.ico`: the app's own file, and the bytes the framework answers with when there is none |
| `runtime-hooks.ts` | the pipeline's `authorize` seam, decided from the app's own `Policy` objects |
| `runtime-replica.ts` | which boot gets a standby, and the one middleware frame that opens the read scope |
| `role-replicator.ts` | the `replicator` role: the feed selected, locked and pumped — and `replicatedRelations()`, the entity TABLES it filters on |
| `role-start.ts` | `--role` selection plus start/stop for `web`, `sync`, `worker`, `scheduler` |
...
| Mechanism | Where | Was |
|---|---|---|
| the transactional outbox | `runtime-queue.ts` installs the store + facade, `worker` runs the relay | staged rows nothing published |
...
| the Postgres event bus | `runtime-queue.ts` | `step.waitForEvent` forgot every correlation on restart |
| the shared idempotency store | `runtime-queue.ts` | a retry on another replica charged the card twice |
...
| the retention sweep | `runtime-purge.ts`, declared in `startServices` | three `purgeExpired()` with no caller — every row `x_idempotency`, `x_rate_limit` and `x_auth_*` ever took was kept |
| the cache tiers | `runtime-cache.ts` | only the CDN tier was registered; memo, LRU and Redis had zero callers |
| WebSocket authentication | `role-sync.ts` | `actorId: null` on every socket — realtime was single-tenant by wiring |
...
`replicatedRelations()` (`role-replicator.ts`) is the one projection, and both of its readers are
...
`examples/dummy` have `name === table` — `role-replicator.test.ts` uses `billingAccount` on
...
`runtime-render.ts` puts `@ultimat3/render`'s `clientScopeTag(clientScopeOf(ctx.actor))` (`@ultimat3/auth`) in the head of a
...
scope to the next. `runtime-render-scope.test.ts` drives `static`, `isr`, `ssr` (gated and not) and `stream` over the real pipeline. There is
...
`errors` and, `As of 2026-09-23`, `manifest` — start when `live` does and are joined before the first step after `eval` (DX ledger #14).
...
| `runtime-render.ts` | one collector **per render**, and `hydrateRuntime` after the body |
...
`role-start-script-csp.test.ts`, which parses a served document, hashes every executable inline
...
`registerRoute` replaces the entry for the same file, and `runtime-render.ts` reads the entry back from
...
### `runtime-assets.ts` is where the image pipeline meets HTTP
...
disk, so `/media/*key` declares what `runtime-storage.ts` declares — `auth: 'required'` +
