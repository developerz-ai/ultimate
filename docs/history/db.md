...
The reasoning moved out of [`packages/db/CLAUDE.md`](../../packages/db/CLAUDE.md) (plan 101,
...
transaction on **this client** is **live** (`liveTxConnection()`) skips the queue because it is already inside the transaction
...
**The second rule fences on `liveTxConnection()`, never on `currentTx() !== undefined`** — the two are
...
false on every exit and `liveTxConnection()` (`transaction.ts`) is the one reader. A closed scope falls
...
**And it fences on the transaction being THIS client's, `As of 2026-09-23`.** `inLiveTx()` asked
"is any transaction open", so with two embedded clients in one process an autocommit write on A,
issued inside a transaction on B, skipped A's queue and ran on A's single session while A's own
transaction held it — rolled back with that transaction. `liveTxConnection()` answers the
connection the live transaction runs on, and `pglite.ts` skips its queue only when that connection
is one of the reservations it issued. `pglite-two-clients.test.ts` pins it on real PGlite.
...
its table from the comparison — the post-migrate drift check (written here as "x db drift", which is no command) answering `ok: true` against a database that still has
...
- **Every numeric option this package bounds anything with is screened, `As of 2026-08-26`** —
  ...
  unbounded wait) and `readonlyQuery`'s `timeoutMs`, plus `client.ts`'s pool profile — and
  `reapBranches`' `maxAgeMs`, `As of 2026-09-23`: `now - NaN` is NaN and `createdAtMs > NaN` is
  false, the "old enough" answer, so `maxAgeMs: NaN` dropped every branch of this database. The last one
...
- **A JS array bound as a parameter is rendered here, because `Bun.SQL` does not render it,
  ...
  `array-parameter.ts` renders the literal, `bound-parameters.ts` is the encoder (arrays and, since `prepare: false`, `Date`s) and `sendOn` (`statement-funnel.ts`) is its one caller — this
