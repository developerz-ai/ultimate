# @ultimat3/query — history

The reasoning moved out of [`packages/query/CLAUDE.md`](../../packages/query/CLAUDE.md) (plan 101,
...
- **The keyset tiebreak keeps the id's TYPE, `As of 2026-09-23`.** `SeekKey.id` is `unknown`
  (it was `string`): `seekKeyOf` stringified it, and `isAfterKey` then compared a numeric id with
  `compareValues` against that string — lexically, so `"10" < "7"` and a row tied on the sort key
  after id 7 was never served (`pagination-id-type.test.ts`). The cursor carries the typed id at
  the TAIL of `key`, through `serializeSortValue`, because core's `CursorPayload.id` slot is a
  string; a cursor minted before this, with no tail, still resumes from that string slot.

- **Equality treats a `number` and a `bigint` as one family** (`shape.ts` `same`): an `int8`
  arrives as `5n` from one driver while a filter literal is `5`, and Postgres answers `5n = 5`
  true. `'5'` still does not equal `5`.
