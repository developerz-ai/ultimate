# @ultimat3/core — history

The reasoning moved out of [`packages/core/CLAUDE.md`](../../packages/core/CLAUDE.md) (plan 101,
...
**A readiness grace runs before the `accept` phase, `As of 2026-09-23`** (`lifecycle-grace.ts`).
Closing the listener the instant `/readyz` flipped left Kubernetes endpoints pointing at a closed
socket, and a POST in that window got a 502. `readinessGraceMs` — `drain.readinessGraceMs` in
`app.config.ts`, handed over by `@ultimat3/http`'s `ServerOptions.drain` — is waited out with
`/readyz` at 503 and the socket open, and it is **added** to `deadlineMs`, never taken from it: the
deadline starts after the grace, so a chart's `terminationGracePeriodSeconds` must exceed
`readinessGraceMs + deadlineMs` (5 s + 25 s by default). Unset, it is 0 in development/test and
5000 everywhere else, **including a process naming no environment** — fail closed, the same
`fallback: 'production'` `assertNoDevSecretsOutsideLocal()` (`dev-secrets.ts`) reads.
