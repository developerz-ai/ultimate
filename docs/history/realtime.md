# @ultimat3/realtime — history

The reasoning moved out of [`packages/realtime/CLAUDE.md`](../../packages/realtime/CLAUDE.md) (plan 101,
...
- One place turns `realtime.transport` + the environment into a bus, and it is
  `selectTransport(env, realtime)` — a boot that resolved the bus itself could reach a different one
  than the container it is standing in for. The KV bucket and the presence TTL come back with the
  transport for the same reason: they are one decision. **The config decides, the env supplies**
  (`As of 2026-09-23`, 22.0.0): `'nats'` dials the variable `urlEnv` names and refuses with
  `X_CONFIG_INVALID` when it is unset; `'memory'` refuses a set `NATS_URL` (or the named variable)
  the same way, because either silent resolution is a node that reaches no other node.
...
- **There is ONE `backoffDelay`, `@ultimat3/core`'s, counted from 1** (`As of 2026-09-23`, 22.0.0).
  Until then `.` exported a 0-based copy that added 1 before delegating: two counting conventions
  under one name, and the channel catch-up retry passed its 1-based failure count straight into it,
  so its first retry waited twice the base. Inside the package, `policyDelay(policy, attempt, rng)`
  maps a `BackoffPolicy` onto core's call and takes core's 1-based count; it is not on the barrel.
  A caller with a 0-based counter adds 1 at the call site (`client.ts`, `socket-engine.ts`,
  `replicator.ts`'s `retryDelayMs`, whose own contract stays 0-based), where the shift is visible.
  `client-reconnect.test.ts`, `socket-engine-reconnect.test.ts` and `nats-transport.test.ts` pin the
  first wait at the base.
...
- Deny by default on topics. No declaration = `X_TOPIC_FORBIDDEN`, and **no declaration without a
  policy**, `As of 2026-09-23` (22.0.0): `channel()`'s `policy` is required by the type and refused
  at runtime (`X_CHANNEL_DECLARATION_INVALID`). It was optional, and an omitted one let any socket —
  anonymous included — join with any param and receive every committed row. A public channel says
  `policy: allow('public')`, as an `action` and a `query` do.
...
- **`#persist` hands the store SNAPSHOTS, never the live entries, and BY KEY.** `QueueStore.write`
  is a durable write (IndexedDB) and may await before it reads. Given an entry itself, a store that
  resolves after the next pass has moved on persists a status that was never true when it was
  called. By key (`QueueChange`: puts, deletes, the sequence floor), never the whole queue, since
  22.0.0: two tabs of one user each saved their own copy, and the last save erased the other's
  write. One tab drains at a time (`navigator.locks`, `page-outbox.ts`'s `exclusive`) and re-reads
  the queue under the lock, which is also where a stored `inflight` — only a pass holding that
  lock could have put one on the wire — goes back to `pending`.
