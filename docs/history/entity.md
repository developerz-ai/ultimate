# @ultimat3/entity — history

The reasoning moved out of [`packages/entity/CLAUDE.md`](../../packages/entity/CLAUDE.md) (plan 101,
...
- **`write-parity.test.ts` runs every write against memory AND a real embedded Postgres (PGlite),
  `As of 2026-09-23`** — not a recording client. Each case is one body run through both drivers
  and the two outcomes compared. What it pins, each found failing first:
  - a patch property present with `undefined` is ABSENT (`bindValues` and memory's `defined()`);
    only `null` clears a column. Before, both drivers wrote NULL — optional action input wiped data;
  - memory refuses a duplicate primary key, a duplicate non-partial `unique` index and a PK patch
    onto another row's key with `X_DB_UNIQUE_VIOLATION` (`memory-unique.ts`, built by
    `@ultimat3/db`'s `driverError` so the shape is Postgres's), and a PK patch leaves ONE row.
    Partial unique indexes are NOT checked in memory: their predicate is SQL this driver cannot
    evaluate, and guessing would refuse a row Postgres accepts;
  - `.transition()` filters on `singleKeyOf(entity)`, never the literal `id`;
  - `decimal()`/`bigint()` store Postgres's spelling: no leading zeros, no `-0`, a short fraction
    padded to the declared scale. Never rounded — excess scale stays refused;
  - a seed `upsert` compares only the columns the caller NAMED, so a table-generated id or
    `defaultNow()` no longer turns every re-run into `'updated'`;
  - `text({ max })` counts code points in `$parse` (as `char_length`), `integer()` holds int4,
    `url()` stores its scheme lower-cased (the CHECK is case-sensitive), and a non-integer `max`
    is refused at declaration.

- **`min`/`max` over a `timestamptz` crosses as epoch milliseconds** (`pg-sql.ts`,
  `aggregate-decode.ts`), never the session's `::text`, which `new Date` read as NaN for an offset
  with seconds and as 1999 for year 0099. `aggregate-time-parity.test.ts` runs it under four
  session zones. Reading a WHOLE ROW under a non-UTC session is a separate path and still fails
  (`X_INVARIANT_VIOLATED`, "an invalid Date") on PGlite — recorded, not fixed here.

- **Text ordering: byte (`C`) order is the parity target, decided 2026-09-23 (plan 101, 03 i).**
  Memory compares text by UTF-16 code unit, PGlite's database is `datcollate = C`, and the two
  agree for every BMP string. A production server created with a linguistic collation
  (`en_US.utf8`) orders `'a' < 'B'` where both of those order `'B' < 'a'` — a known divergence,
  not a bug either driver can fix alone. NOT refused: text `orderBy` is the commonest listing
  there is, and refusing it the way `aggregate.ts` refuses text `min`/`max` breaks every app.
  NOT pinned with `collate "C"` in the generated SQL: that defeats every btree index on the
  column for `ORDER BY`, turning a keyset seek into a sort. Create the production database with
  `LC_COLLATE=C` when byte order must hold everywhere.
