# @ultimat3/jobs — history

The reasoning moved out of [`packages/jobs/CLAUDE.md`](../../packages/jobs/CLAUDE.md) (plan 101,
...
- **`run-once` fires ONE catch-up, and the watermark is what makes it one.** `dispatch()` marks
  ...
  `skip` fires the TRUE latest missed occurrence, found by bisection (`latestOccurrenceBy`,
  `scheduler.ts`), never `due`'s last element — that one is `maxCatchUp` steps past the watermark,
  and dispatching it left the next tick with ten more still due. This line said "within the cap"
  until 2026-09-23, after the code had stopped doing it.
