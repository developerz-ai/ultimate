...
| [`01-package-map.md`](./01-package-map.md) | 30 packages, 6 tiers, one reason to change each. What every package owns and must never do. |
...
One command runs all of them, from one step list: `bun run verify` in this repo *is* `x verify` run at the repo root — same steps, same report, same exit code. `--only <step>` runs one step for an iteration loop and labels itself `NOT A GATE RUN`; there is no `--skip`.
