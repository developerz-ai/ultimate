...
22 packages, 6 tiers. **One package = one reason to change.** If two things in a package change for different reasons, they are two packages.
...
```
tier 0  core, schema
tier 1  i18n, money, time, cache, seo          (may import tier 0)
tier 2  entity, policy, http                   (may import tier 0-1)
tier 3  action, query, jobs, realtime          (may import tier 0-2)
tier 4  render, pwa, mcp, ai, manifest         (may import tier 0-3)
...
| `entity` | 2 | a table + its domain type + invariants the DB also enforces | column types, defaults, invariants, tenant column, repo factory, cursor codec | business logic; HTTP awareness; policy decisions |
