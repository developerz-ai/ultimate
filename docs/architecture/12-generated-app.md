...
| # | File | Contents |
|---|---|---|
| 1 | `packages/db/src/schema/posts.ts` | Drizzle table with tenant column, timestamps, indexes |
| 2 | `packages/db/migrations/0007_create_posts.sql` | generated by the same diff as `x db gen` |
| 3 | `apps/web/app/posts/entity.ts` | `entity(posts, { tenant, invariants })` + `PostView` |
| 4 | `apps/web/app/posts/repo.ts` | cursor-paginated `list`, `byId`, `insert`, `update` |
| 5 | `apps/web/app/posts/service.ts` | one method per use case, no HTTP |
| 6 | `apps/web/app/posts/policy.ts` | `post:read` / `post:write` / `post:publish` with denial reasons |
| 7 | `apps/web/api/posts.ts` | CRUD actions with `input`, `output`, `policy`, `cache.invalidates`, `mcp` |
| 8 | `apps/web/app/posts/live.ts` | `query({ live: true })` with total order + limit (`--live`) |
| 9 | `apps/web/app/posts/jobs.ts` | one stub job with a required `idempotencyKey` |
| 10 | `apps/web/app/posts/ui/{post-card,post-form,post-list}.tsx` | components, tokens only, `t()` only |
| 11 | `apps/web/app/posts/page.tsx` + `defineRoute` | `render`, `hydrate`, `offline`, `budget`, `meta` |
| 12 | `packages/i18n/en/posts.json` (+ every configured locale) | every key the components use — so the build is green |
| 13 | `apps/admin/posts/*` | admin screens derived from the entity (`--admin`) |
| 14 | `packages/mcp/src/posts.ts` | the app's MCP entries for the exposed actions |
| 15 | `*.test.ts` next to each | contract, live, job, and route scaffolds that **fail until filled in** |
| 16 | `x.manifest.json` | regenerated |

Every generated file is real, typed, and passes `x verify` except the test scaffolds, which fail on purpose: an untested resource is a red build, not a backlog item.
