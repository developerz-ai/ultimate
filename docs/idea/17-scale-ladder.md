...
**`x new` ships this file now** — `docker/docker-compose.prod.yml`, alongside `docker/Dockerfile` and its `.dockerignore` — and, since 4.0.0, `docker/helm` too, so reaching rung 3 is `x deploy --method helm` rather than a copy.

**A restart on this rung is a restart**, `As of 2026-09-23`, and that is the rung rather than a gap. `web` and `sync` each hold the one binder of a published port, so there is no second replica to take traffic while the first drains: the readiness grace and the drain finish what is in flight, and requests that arrive in between wait or fail. Invisible rolling restarts are a **Kubernetes** claim, proved by `deploy-proof` ([`14-roadmap.md`](./14-roadmap.md), milestone 11). A reverse proxy in front of two `web` services is the app's to add; the framework ships none (axiom 7).

**Every app-image service is hardened and rotates its logs**, `As of 2026-09-23` — one `x-common` block in both compose files: `read_only: true` with tmpfs for `/tmp` and `.x/`, `cap_drop: [ALL]`, `no-new-privileges`, a `mem_limit`, and `json-file` logs capped at 3 × 10 MB per container. Docker's default driver never rotates, and on this rung the disk it fills is the database's.
...
4. **Config keys that are accepted and ignored are now a gate, not a habit** — `bun run scripts/config-readers.ts` refuses an `AppConfig` leaf key with no reader in `packages/*/src`. Twelve had shipped before it existed, each found by hand in a major: `jobs.driver` (5.0.0), `realtime.heartbeatMs` / `database.urlEnv` / `database.poolSize` / `database.schema` (4.0.0), `pwa.installPrompt` / `auth.afterSignInPath` / `ai.modelEnv` (2026-08). What is left is a pin list with a sentence per key (`scripts/lib/config-reader-pins.ts`). `realtime.urlEnv`, `realtime.transport` and `realtime.enabled` were the last realtime entries and 22.0.0 spent them by **wiring** rather than deleting: the boot selects the bus from `transport`, dials the variable `urlEnv` names, and starts no `sync` node or replicator under `enabled: false`. `realtime.transport: 'redis'` is gone from the type and refused by `defineConfig` — it built in-process or NATS, never Redis. `cache.urlEnv` was the other one and 9.0.0 spent it, with `cache.driver`.
5. **The bus is declared in `app.config.ts` and supplied by env, as of 22.0.0.** `selectTransport(env, realtime)` builds what `realtime.transport` says and reads its url from the variable `realtime.urlEnv` names; a config and an environment that disagree refuse the boot (`X_CONFIG_INVALID`) rather than one silently winning. Until then `NATS_URL` alone decided and the config field documented intent. The change feed is still env-selected — `selectChangeFeed(env)` reads `DATABASE_URL`/`REPLICATION_URL` — because there is no feed choice to declare: a real database has a log and an embedded one does not.
...
| A Helm chart with per-role HPAs | **true, and emitted by `x new`** since 4.0.0 — the worker's `queue_depth` is an `External` metric, because every worker publishes the same global backlog |
...
| a rolling restart on Kubernetes drops no request | **measured green locally, not yet on CI** — `deploy-proof` on kind, `As of 2026-09-23`: 0 of 8,686 requests failed across `helm upgrade` (it was 695 of 8,692 `X_DRAINING` 503s before the HTTP drain fix). Roadmap milestone 11 ([`14-roadmap.md`](./14-roadmap.md)) |
| a restart on Compose invisible | **not claimed** — one published port, one binder; rung 2 restarts |
