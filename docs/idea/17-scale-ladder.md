...
4. **Config keys that are accepted and ignored are now a gate, not a habit** — `bun run scripts/config-readers.ts` refuses an `AppConfig` leaf key with no reader in `packages/*/src`. Twelve had shipped before it existed, each found by hand in a major: `jobs.driver` (5.0.0), `realtime.heartbeatMs` / `database.urlEnv` / `database.poolSize` / `database.schema` (4.0.0), `pwa.installPrompt` / `auth.afterSignInPath` / `ai.modelEnv` (2026-08). What is left is a pin list with a sentence per key (`scripts/lib/config-reader-pins.ts`), and two entries on it are the same defect not yet spent: `cache.urlEnv` and `realtime.urlEnv` are **validated and never dereferenced** — `dev-cache.ts` reads the literal `REDIS_URL` and `dev-services.ts` the literal `NATS_URL`, so `urlEnv: 'MY_REDIS'` makes nothing read `MY_REDIS`. They are pinned rather than deleted because deleting a config key is breaking and belongs to a release. `realtime.transport: 'redis'` still type-checks and still builds in-process or NATS, never Redis.
