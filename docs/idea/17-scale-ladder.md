...
Observability is the part that is not optional at this rung — and the part the framework does least of. See [Where the invariant breaks today](#where-the-invariant-breaks-today), item 3.
...
|---|---|---|---|---|---|
| Rows / SQL | `@ultimat3/db` · `DbClient`, `ReservableClient` | `createPostgresClient()` over `Bun.SQL`; `setDbClient()` overrides | `database.urlEnv`, `database.poolSize` | `DATABASE_URL` | shipped |
...
The framework's shared tier uses `GET`, `SET … EX`, `SADD`, `DEL`, `SMEMBERS` and one `EVAL` ([`packages/cache/src/redis.ts`](../../packages/cache/src/redis.ts)). Five core commands, and one script that does not survive the move.

### The blocker: `invalidateTags` uses undeclared keys

Dragonfly enforces that a Lua script may only touch keys passed in `KEYS` — `CheckKeysDeclared()`, rejected otherwise ([docs](https://www.dragonflydb.io/docs/managing-dragonfly/scripting)). `REDIS_INVALIDATE_SCRIPT` passes only the **tag-set** keys, then `SMEMBERS` each one and `DEL`s the members it finds at runtime. Those members are never declared.

**So the shared cache tier's invalidation path fails on Dragonfly out of the box — single node, not just cluster mode.** Two escapes, both with a cost:

| Escape | Cost |
|---|---|
| `allow-undeclared-keys` (script flag, `--default_lua_flags`, or `SCRIPT FLAGS`) | Dragonfly's own docs: *"Dragonfly has to stop all other operations when the script is running."* A global lock on every tag invalidation |
| rewrite `invalidateTags` to `SMEMBERS` client-side, then `DEL` in a declared batch | one extra round trip, and loses the atomicity the script exists to provide |

Neither is app code — both are `packages/cache/src/redis.ts`. **Until one lands, treat Dragonfly as untested for this framework's shared tier and keep `REDIS_URL` pointed at Redis or Valkey.**

**Cluster mode is separately unusable.** `--cluster_mode=yes` forbids `SELECT`, rejects global `PUBLISH`/`SUBSCRIBE`, and requires every key of a multi-key operation, transaction or Lua script to live in one slot ([docs](https://github.com/dragonflydb/dragonfly/blob/main/docs/cluster-mode.md)). The key layout is `x:t:<tag>` with no hash tags, so the tag-set keys will not co-slot — the same invalidation breaks a second way. Dragonfly also ships only the cluster **data plane**: no gossip, no `ASKING`, and failover and slot rebalancing are explicitly out of scope for the open-source server.
...
| Dragonfly at all | `invalidateTags` declares its keys. Redis and Valkey work today; Dragonfly does not |
...
| shared cache tier over the Redis protocol, tag sets, single-trip invalidation | **true** — `packages/cache/src/redis.ts` |
...
| Dragonfly as the shared tier | **breaks** — `invalidateTags` deletes keys it never declared, single node and cluster alike |
