...
Every row is a mechanism that exists and works, whose only missing piece is a call site in [`packages/cli/src/runtime-services.ts`](../../packages/cli/src/runtime-services.ts) or [`role-start.ts`](../../packages/cli/src/role-start.ts).
...
| Shared cache tier | `createRedisTier`, `createLruTier`, `createCacheStack` — **zero callers**. Twelve pods held twelve private LRUs and `x cache bust` cleared one | **closed** — tiers selected from the environment, plus a cross-instance invalidation seam. `As of 2026-08` `createCacheStack` has a production caller: a `cache:` query's read fills through it (`packages/query/src/cache.ts`), so the read path and the bust path are one registry |
...
**The root cause was one missing field, and it is closed** `As of 2026-08-23`. `ServeOptions` was
`{ root, env, role?, port?, metricsPort? }` — nowhere to hand the framework a driver — and the one
override path that existed was a race: `startServices` captured its own drivers into
`RunningServices`, *then* `loadApp` imported the app module calling `setJobDriver(theirs)`. The
ambient slot changed and the captured object did not, so **`enqueue()` wrote to their queue while
the worker claimed from Postgres, silently**, with the `/_x` panel reading the ambient driver and
agreeing with the wrong half.

`ServeOptions.runtime` is the field ([`cli/src/serve.ts:146`](../../packages/cli/src/serve.ts)),
typed `RuntimeOverrides` ([`cli/src/runtime-overrides.ts:26`](../../packages/cli/src/runtime-overrides.ts)):
`jobs`, `storage`, `mail`, `transport`, `purge`, `rateLimitStore`, `isrStore`, `middleware`,
`images`, `syncAuthenticate`. A driver handed in is installed as the ambient one too, and
`startRoles` refuses a boot where the two disagree — the split is now a refusal, not a silence. What
is still absent is the row below: an app cannot contribute a raw `Route`.
...
| Their queue | **Yes**, `As of 2026-08-23` — `ServeOptions.runtime.jobs` installs it as the driver every enqueue AND every claim uses, and `startRoles` refuses to boot when the two disagree ([`cli/src/runtime-overrides.ts:31`](../../packages/cli/src/runtime-overrides.ts)). The config key this row named is **gone**: `JobsConfig.driver` accepted `'postgres' \| 'redis' \| 'nats'`, was read by nothing and always gave you Postgres, so 5.0.0 deleted it ([`core/src/config.ts:96`](../../packages/core/src/config.ts)) |
| **Their enterprise IdP, over OIDC** | **Now yes, for OIDC specifically.** It was a closed union of `github \| google \| apple` — Okta, Entra and Ping were unrepresentable, and the constraint was a **type**, so there was no runtime escape. A provider now registers, or `discoverOAuthProvider({ id, issuer })` reads one out of the issuer's discovery document; PKCE stays the literal `true`, so the opening cannot be used to register a PKCE-less provider. **SAML does not ship and is permanently out of scope** — XML-DSig canonicalisation has no Bun native and implementing it would put a real dependency in the primitive vocabulary, which [`18-build-vs-wrap.md`](./18-build-vs-wrap.md) does not permit. The shipped answer is an OIDC-speaking SAML bridge in front, registered as a provider |
...
| Global HTTP middleware, a shared rate-limit store, a shared ISR store | **Yes**, `As of 2026-08-23` — all three are `ServeOptions.runtime` fields (`middleware`, `rateLimitStore`, `isrStore`), and the boot now passes them. A shared rate-limit store also **ships**: omit the field and the boot installs `postgresRateLimitStore` over the pool it already opened ([`cli/src/runtime-services.ts:307`](../../packages/cli/src/runtime-services.ts)), so `scope: 'shared'` is satisfiable rather than merely declarable. A store whose scope is `'process'` is legal and warned about |
...
|---|---|---|
| ~~Notifications — channel fan-out, preference gate, digest window, delivery ledger, in-app inbox~~ | **Shipped `As of 2026-08-24`** — `notifier()`, a job factory in the new `@ultimat3/notify` (tier 4) | Both tracked apps already hand-rolled it: 484 lines in the demo, 250 more in the reference app, structurally identical. The notification *taxonomy* and `quietHours` must never ship |
| ~~Entity-level full-text search~~ (faceting deferred) | **Shipped `As of 2026-08-24`** — `searchable()` + `search()`, a query factory | The framework already contains a correct FTS + RRF implementation — bound to the RAG table and unreachable from an entity. A hand-rolled `to_tsquery` from user input is an injection or a seq scan. `websearch_to_tsquery`, never bare `to_tsquery` (which reads a user's `&`, `|`, `!`, `:*` and parens as OPERATORS) and never `plainto_tsquery` (which silently discards `"a phrase"` and `-negation`). **Deferred, each named**: relevance ordering (`ts_rank` is computed, so making it seekable spans four entity files — and shipping `order by ts_rank` WITHOUT the cursor half is exactly the pager that drops rows, which 12.0.0 just fixed), faceting, paging past page one at the query layer, and a composite `(tenant, tsvector)` GIN, which needs `btree_gin` and an opclass `IndexInit` has no spelling for |
| ~~Outbound webhooks + inbound verifier~~ | **Shipped `As of 2026-08-24`** — `webhook()` (job factory) + `verifyWebhookSignature()` | [`19-mechanism-not-convention.md`](./19-mechanism-not-convention.md) names this by hand as a mechanism the framework should own. Neither direction ships today |
| ~~Form binding to an action's input schema~~ | **Shipped `As of 2026-08-24`** — `useForm()` in `@ultimat3/ui`, and the wire now carries the issue list | Every ingredient exists and none are connected: the schema, `safeParse`'s issue paths, the typed client, `Field`'s error slot. The 40th form in a monolith is 40 hand-written issue-path mappings |
| ~~Async export of a large dataset~~ | **Shipped `As of 2026-08-24`** — `exportRows()`, a job factory | Streaming a paged read to object storage with a resumable cursor is identical for a bank and a blog. The *columns* are the app's |
| ~~Durable admin audit sink~~ | **Shipped `As of 2026-08-24`** | `postgresAuditSink({ executor })` in `@ultimat3/action` — one append-only `x_audit` row per record, no `on conflict` (two identical attempts are two events), two indexes and **no purge**: retention is seven years for one app and thirty days for the next, so shipping a `delete` would be shipping one of those answers. `memoryAuditSink()` stays the dev sink and is honest about it — a bounded ring (`DEFAULT_MAX_AUDIT_RECORDS`, 1,000) that evicts oldest-first and publishes `size` and `dropped`, so "it discards" is a number in a running process rather than a sentence in a header. The sink never walks the `Ctx`: `createContext` spreads every installed service onto it and an HTTP surface's value is a `RequestContext` carrying the caller's `Authorization` and `Cookie`, so the row is a fixed ALLOW-LIST (`requestId`, `traceId`, `locale`, `tz`, `buildId`, `role`, and the actor's `id`/`kind`/`orgId`/`onBehalfOf`) with `input` redacted through core's own `isRedactedKey`. **The gap this row claimed is closed, and it was never open**: this sentence said nothing applied `SQL_AUDIT_TABLE` at boot, and `applySchema` had been installing it, with an argued comment, since before the row was written. Corrected 2026-08-24 by the audit that enumerated every framework table against its applier — which found the real hole one row over, in `@ultimat3/auth` |
| ~~State machine over an entity column~~ | **Shipped `As of 2026-08-24`** — `enumerated().transitions()` + `transition()`, a mutator factory | An illegal transition is a defect in every business. An *approval chain* is not — that stays the app's |
| ~~Streaming and a tool loop from `llm()`~~ | **Shipped `As of 2026-08`** | `.stream()` on `llm()`, and the loop as `agent()` — a factory over `action()`, with `hive()` for fan-out and `agentJob()` for the queue. Without them the first agentic feature called the gateway directly and lost policy, budget, schema, span and manifest row |
| ~~Read replicas~~ | **Shipped `As of 2026-08-24`** | `DATABASE_REPLICA_URL` builds a primary + replica `DbClient` ([`db/src/default-client.ts`](../../packages/db/src/default-client.ts)) and `withReplicaReads(fn)` is the scope a plain read may leave the primary inside. Opt in **twice**, and the second one is the safety argument: `packages/db` cannot see a request boundary, so a write-marker keyed on `requestId` would be a map that only grows and any eviction that forgot a scope that WROTE would serve it a stale row. Read-your-writes is one mutable bit on the scope — one write at any depth pins the rest of it to the primary — transactions are always the primary's, and three consecutive replica failures park it for ten seconds. **Nothing opens the scope yet**: wrapping a request in it is the app's line |
...
| **Many big servers** | Idempotency, rate limits and job concurrency all silently multiply by replica count; cache invalidation clears one pod; a cross-node reconnect always full-snapshots; the scheduler double-fires during every rolling restart | **The app must say which, and a shared store ships for rate limits and auth lockouts** (`postgresRateLimitStore`, `postgresAuthLimiter`, `As of 2026-08`) and must be installed deliberately. `rateLimit.scope` no longer defaults to `'process'` — an enabled limiter with no scope is `X_RATE_LIMIT_SCOPE_UNSET` at `defineHttpConfig`, and a framework-booted process DERIVES it from the store it installed rather than guessing — and `assertRateLimitScope` then refuses a `'shared'` declaration the installed store cannot satisfy, which until now meant `scope: 'shared'` was undeployable rather than silently wrong. Idempotency, job concurrency, cache invalidation and the scheduler still multiply per process |
| **Datacenter** | Change fanout is still at-most-once over core NATS — a blip is changes this node never saw. It is now **detected**, `As of 2026-08`: the replicator stamps `producer` + `seq` on every envelope ([`realtime/src/replicator.ts`](../../packages/realtime/src/replicator.ts)), `SeqGapDetector` sees the skip, and the sync node answers it with `registry.invalidate()` — every window desynced, every subscriber re-served on the next change to its query. What is left is the **cost**: a gap is repaired by re-snapshotting, not by replay, so a datacenter blip is one bounded query per live subscription | detection, not delivery. `channel` topics have no cursor and no re-snapshot at all — a frame `SyncSocket.send` drops under backpressure is counted (`channel_frames_dropped_total`) and unrepairable |

**The 50,000-socket benchmark measures the first three rungs, not the fourth.** The bench server registers zero live queries, has no database, no policy, no presence and no auth, and never crosses NATS — so *"156,851 attempts shed before any query path"* is true because there is no query path. What it proves is Bun's accept path, the `AcceptBudget` and channel fanout over an in-process transport, recovering from `SIGKILL`. The number that matters for a large app is the other one: **p90 time to the first patch on the reconnected socket, 105 seconds**, budget-bound, on one node. That is reachability; whether anything was *lost* is the delivery run's question, and it is answered only at 10,000 clients — 1,666,882 patches, 0 observed sequence gaps (a lower bound — a hole is only visible between two frames one connection received), `As of 2026-08`. [`scripts/bench/restart-bench.ts`](../../scripts/bench/restart-bench.ts).
...
| `x verify` green = shippable | **true of the framework**; no Ultimate *application* in this repo is green — 4 of 20 steps pinned red on the reference app (`typecheck`, `e2e`, `drift`, `budgets`), 2 on the deployed demo (`boundaries`, `budgets`) ([`scripts/lib/gated-apps.ts`](../../scripts/lib/gated-apps.ts)) |
...
| Rate limits and idempotency enforce the number they declare | **true where declared** — both refuse at boot rather than silently multiplying by replica count, and **both shared stores now ship**: `postgresRateLimitStore` is the boot's default over the pool it already opened, so `scope: 'shared'` is satisfiable ([`cli/src/runtime-services.ts:307`](../../packages/cli/src/runtime-services.ts)). `As of 2026-08-23` |
...
| Found | Was |
|---|---|
| `AUTH_TABLES` — `x_users`, `x_sessions`, `x_accounts`, `x_verifications`, `x_api_keys` | declared, exported, and created by **nothing**, in dev or production, from the initial commit through all 21 released versions. Not `entity()` declarations, so `x db gen` never saw them; the reference app recorded the symptom — nobody could hold a session — without anyone reaching the cause |
| `defineService` | installed on jobs, tasks and CLI commands and **not on HTTP**, so `ctx.posts` was `undefined` on the surface an app spends its life on — and `ctx.posts` is the spelling [`15-adding-a-feature.md`](../architecture/15-adding-a-feature.md) writes in its worked example. The documented path was the broken one |
| `X_TEST_SCHEMA_EXPECTED`, `X_TEST_JOB_EXPECTED` | declared, registered and titled, and unreachable by any caller: Bun **replaces** an error thrown from an `async` matcher body with its own message |

Each is now mechanised rather than fixed — `bun run framework-tables` for the first, composition for the second (`createRequestContext` calls `createContext()` instead of building a second context beside it, so both halves move together or neither does). The lesson generalises past these three: **the gap register itself carried a false row**, claiming nothing applied `SQL_AUDIT_TABLE` when `applySchema` had installed it for releases. A register nobody re-derives is a declaration nothing reads.
