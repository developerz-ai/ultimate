...
| **Many big servers** | Idempotency, rate limits and job concurrency all silently multiply by replica count; cache invalidation clears one pod; a cross-node reconnect always full-snapshots; the scheduler double-fires during every rolling restart | **Per-process defaults; a shared store ships for rate limits and auth lockouts** (`postgresRateLimitStore`, `postgresAuthLimiter`, `As of 2026-08-22`) and must be installed deliberately. `assertRateLimitScope` refuses a `'shared'` declaration the installed store cannot satisfy — which until now meant `scope: 'shared'` was undeployable rather than silently wrong. Idempotency, job concurrency, cache invalidation and the scheduler are unchanged |
