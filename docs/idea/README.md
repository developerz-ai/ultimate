...
| [`21-the-range.md`](./21-the-range.md) | **Who it is for, and the range**: a homework assignment to a very large product, one framework, no lite mode. The small end measured (0 questions, 151 files, 4 commands, and the scaffold's own `bin/setup && bin/check` green on the first pass — no waiver, no fix-follow); the large end anchored to the ladder, the tiers and the gate; and why enforced conventions pay MORE with a cheap model. |
