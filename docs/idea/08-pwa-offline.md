...
| 5 | **Forced reload after a grace period** | **deleted, not designed, `As of 2026-08-23`.** It was "designed, not wired" for four majors; 9.0.0 removed the declarations rather than wiring them, because the two runtimes holding both build ids (`http` tier 2, `sync` tier 3) are below `pwa` (tier 4) and could never call in. Notification ships and is complete; forcing does not |
