...
| Generated | Detail |
|---|---|
| `manifest.webmanifest` | name, short_name, description, `start_url`, `scope`, display, theme + background from `pwa.colors` — **emitted**, `As of 2026-08-27` ([#362](https://github.com/developerz-ai/ultimate/issues/362)); every other row on this page is still designed, not shipped |
...
| 5 | **Forced reload after a grace period** | **deleted, not designed, `As of 2026-08`.** It was "designed, not wired" for four majors; 9.0.0 removed the declarations rather than wiring them, because the two runtimes holding both build ids (`http` tier 2, `sync` tier 3) are below `pwa` (tier 4) and could never call in. Notification ships and is complete; forcing does not |
