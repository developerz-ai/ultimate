...
| 8 | 🚧 | **PWA + offline + version skew** | generated `sw.js`, precache derivation, manifest/icons/splash from one source icon, required offline fallback, immutable build ID, N-deploy retention, `AppUpdateAvailable` | **HALF DONE, `As of 2026-08-27`.** The manifest half shipped: `x dev`, the container and the static export emit `manifest.webmanifest` and the `<head>` that names it, so an app is installable ([#362](https://github.com/developerz-ai/ultimate/issues/362), `packages/cli/src/pwa-artifacts.ts`). **No build emits a `sw.js`** — `generateServiceWorker` and `buildPrecacheManifest` still have no caller, so "works offline" and "six deploys with a tab left open never 404s a chunk" are undemonstrated. This row read ✅ until 2026-08-27, on a library nothing called. Reading the client build-ID spread back is `x status`, still **planned** — it is in the registry and exits `X_NOT_IMPLEMENTED` ([`12-build-deploy.md`](./12-build-deploy.md)) |
...
| 8 | installable (**done**, `As of 2026-08-27`); works offline and survives six deploys with a tab left open (**not demonstrated** — no build emits a service worker, [#362](https://github.com/developerz-ai/ultimate/issues/362)) |
