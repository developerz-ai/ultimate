...
**Closed since**: the two-platform deploy proof (milestone 11). `.github/workflows/ci.yml`'s `deploy-proof` job (`docker/deploy-proof/run.sh`) scaffolds an app on this tree's packages, installs it into kind with `x deploy --method helm`, and upgrades it under mixed GET/POST load, failing on one non-2xx. **Green on `main`**, `As of 2026-09-24`: run 35937459222 on `c283c112`, **0 of 9,028 requests failed** (2,364 answered by the old build, 6,664 by the new). Compose is the single-node rung and makes no invisible-restart claim ([`17-scale-ladder.md`](17-scale-ladder.md)). The proof runs without `sync`, which ships off by default.
