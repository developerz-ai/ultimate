...
| 10 | ✅ | **Admin + generators + `x new`** | generated admin dashboard (with its own MCP surface), all `x g` generators, `create-ultimate`, `/_x` dev dashboard complete | `bunx create-ultimate myapp && cd myapp && bin/setup && x dev` is <60s with **no Docker and no env editing** — measured `As of 2026-09-11`, 6,802ms for `bin/setup`'s six steps on a warm Bun cache, 12.0s when the install is cold. **Four commands, not three**: `x new` installs nothing, so `x dev` straight after `cd` stops on `X_BUILD_FAILED` naming `bun install`. **`x new`'s output clears the whole gate on the FIRST pass**, `budgets` included, when it is reached through `bin/check` — which is `x build --target static` and then `x verify`, and the build is what writes the `.x/build-stats.json` the `budgets` step weighs. That is how CI asserts it, `As of 2026-09`: `scaffold-smoke` runs the app's own `bin/setup && bin/check` **once, with no waiver and no fix-follow** ([`scripts/scaffold-gate.ts`](../../scripts/scaffold-gate.ts)) — one pass is all a box gets, so a gate that repaired the tree it was measuring could not tell you whether the first run was green. **`lint` is clean on the first run**, zero diagnostics across every source file the scaffold writes, measured `As of 2026-09-11`. It used to depend on the app's name: the templates emitted `@<app>/…` before `@ultimat3/…`, so `x new zebra` started with 4 `organizeImports` findings where `x new alpha` started with 0, until `sortedImports` closed the import-order half ([Known gaps](../../wiki/Known-Gaps.md)). **It is an unmodified green gate** `As of 2026-09`, which it was not before: the allowance that let `budgets` stay red is gone, because a scaffolded app now builds before it verifies and the allowance had nothing left to excuse. `budgets` is asserted **green**, never merely not-red — a skip would mean that static build bought nothing and nobody would see it. The wider claim — **every** `x g` generator's output gating the same way — is covered too `As of 2026-08-22`: the job runs `scripts/scaffold-first-run.ts`, which projects the run from the CLI's own `GENERATORS` registry rather than a sample, then the gate above over the result |
| 11 | ✅ | **Deploy + docs + 1.0** | `x build --target docker\|binary\|static`, `packages/cli/src/serve.ts`, the scaffolded `apps/web/server.ts` + `prerender.ts` + Dockerfile + `docker-compose.prod.yml`, dev/prod compose, Helm with per-role HPAs, graceful drain everywhere | the demo app runs on one Compose box **and** a K8s cluster from one image; on **Kubernetes**, a rolling restart is invisible to connected clients — measured by `.github/workflows/ci.yml`'s `deploy-proof` job (`docker/deploy-proof/run.sh`), which fails on one non-2xx. Compose is the single-node rung and makes no such claim ([`17-scale-ladder.md`](17-scale-ladder.md)): one published port has one binder, so a restart there is a restart |
...
|---|---|
| **Two-platform deploy proof** (milestone 11) | Every artifact the milestone names exists, and `As of 2026-09-23` the **Kubernetes half is a CI job**, `deploy-proof` (`docker/deploy-proof/run.sh`, main only): `x new` on this tree's packages, two images differing only by `BUILD_ID`, `x deploy --method helm` into kind, then an upgrade under mixed GET/POST from inside the cluster, failing on a single non-2xx. **Green locally on this tree, `As of 2026-09-23`**: kind (Kubernetes 1.33), a scaffolded app on this tree's packages, **0 of 8,686 requests failed** across `helm upgrade` (3,516 answered by the old build, 5,170 by the new). It was 695 of 8,692, every one `X_DRAINING` 503, until `@ultimat3/http` stopped refusing requests during the readiness grace and answered them with `Connection: close` instead. **Green on `main`**, `As of 2026-09-24`: ci run 35937459222 on `c283c112`, **0 of 9,028 requests failed** across the rolling upgrade (2,364 answered by the old build, 6,664 by the new) — the evidence the milestone names, so the row is closed. The proof runs without the `sync` role: it has no NATS and no replicator, and a sync pod with neither is refused at boot (`X_REALTIME_TOPOLOGY`). The Compose half is narrowed rather than owed: Compose is the single-node rung, and "invisible restart" is a Kubernetes claim only. Hetzner is no longer named — `x deploy --method compose` against any one box is the same proof |
...
| 8 | installable (**done**); works offline (**done and browser-proven**, `As of 2026-08` — `packages/cli/e2e/service-worker.e2e.test.ts`); survives six deploys with a tab left open (**not demonstrated** — it needs a second build served under a new build id, which no test harness produces) |
...
| 11 | running on Compose and on K8s from one image; rolling restarts invisible on K8s (`deploy-proof`) |
...
| Tier 3 local-first shipped in 21.0.0, opt-in | An entity declares `persist: true` (`packages/entity/src/entity.ts`) and its records outlive a reload: the page's durable store is IndexedDB (`packages/realtime/src/local-store-idb.ts`, memory with one `X_LOCAL_STORE_UNAVAILABLE` warning where IndexedDB is blocked), written by `packages/realtime/src/record-persister.ts`, and writes the network refused wait in the page's outbox and replay in order over HTTP (`page-outbox.ts`). There is no OPFS store and no `local-store.ts` any more; this row described both until 2026-09-23 |
