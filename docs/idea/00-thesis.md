...
| 9 | **Useful, then efficient.** | Working function first, bytes and milliseconds second, so an agent never guesses which wins. A route or island budget is a measurement, not a ceiling to game. Function that costs bytes raises the budget by the measured amount, with the number and the reason in the same diff. A feature cut, stubbed or left `X_NOT_IMPLEMENTED` to hold a figure is the defect, and so are bytes that buy no function. Axiom 6 is not traded. The same-diff half is `bun run budget-raises` (`X_BUDGET_RAISE_UNSTATED`), checked against `origin/main`. |
