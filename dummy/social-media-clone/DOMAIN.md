...
| ~~`follows`~~ | — | **Not built** `As of 2026-08`. Designed here and never written: there is no `packages/db/src/schema/follows.ts`, no migration and no code anywhere referencing `followerId`. The row stays, struck through, because deleting it invites the next reader to re-derive the same table. Visibility in this app is `friendships` + `blocks`, which are built; an asymmetric follow would be a second relation answering the same question |
