...
```sh
x build --target docker --tag social-media-clone:dev
docker run --rm -e ROLE=migrate -e DATABASE_URL=postgres://... social-media-clone:dev
docker run -p 3000:3000 -e DATABASE_URL=postgres://... social-media-clone:dev
```
