 ...
 * | `core -> schema` | five declarations were duplicated on the core side and held equal by 394 lines of pin test in a TIER-5 package — including `describeValue`, which prints INSTEAD of a rejected password, so the safety property of the framework's most security-sensitive renderer rested on a 63-line behavioural pin in `@ultimat3/cli` that no rule required to exist. The lower-tier extraction is what `@ultimat3/schema` ALREADY IS: it imports nothing, it is where `CURRENCY_CODE_PATTERN` reaches the generated Postgres CHECK and the published OpenAPI `pattern`, and there is nowhere below tier 0 for a sixth package to go. The reverse stays forbidden on its own merits and is not weakened by this: `schema -> core` would give `t` a dependency, and `t` is in every bundle graph an app has. Measured before declaring it (2026-08-27, `bun build --target=browser --minify`): a chunk importing only `UltimateError` from core is 6,362 B and one importing `moneyText` from `@ultimat3/ui` is 27,203 B, both of which ALREADY carry schema — `@ultimat3/money` puts it in the graph, which `packages/ui/src/barrel-bytes.test.ts` had already measured and written down |
