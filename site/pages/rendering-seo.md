...
nav: Rendering
description: Five render modes declared per route, a 0kb JS static path, and SEO that fails the build instead of failing quietly in a Lighthouse audit.
lede: Render mode is a per-route property. SEO is enforced by the build, not described in a guide.
updated: 2026-07-26
---
...
`ssr` gives freshness and a slow TTFB — the whole page waits for the slowest query. `spa` gives
an instant shell, a spinner farm, and no HTML for anything. `stream` gives both halves: shell
now, data as it resolves.
...
In a VDOM framework a streaming shell still pays for hydrating the whole tree, which is why
streaming there buys TTFB but not TBT. Here it buys both.

## 0kb JS on the static path

Default for every `site/` route: `render: 'static' | 'isr'`, `hydrate: 'never'`. The HTML ships
with inlined critical CSS and no script tag at all.

| Need | Solution — no framework JS |
|---|---|
| Mobile nav toggle | `<details>` / CSS `:has()` / checkbox |
...
The site you are reading is built the same way: no framework, no bundler runtime, one inline
script for the theme toggle, and its stylesheet inlined at build time.

`site/` cannot import `app/` — a build error, transitively resolved, because that import is how
a marketing page ends up shipping a charting library:
...
Every SEO regression in history was a documented convention someone forgot.
...
`ld.*` helpers are typed against schema.org shapes — `ld.Article`, `ld.Product`,
`ld.Organization`, `ld.BreadcrumbList`, `ld.FAQPage`, `ld.SoftwareApplication`, `ld.Event`,
`ld.Recipe`. A missing required property is a type error, not a Rich Results Test failure
...
|---|---|---|
| `sitemap.xml` | all indexable routes + `prerender()` results | `lastmod` from the entity's `updatedAt`; auto-split at 50k URLs |
...
Deleting a route removes it from the sitemap in the same build. Nothing here is a plugin.
...
| Numbers / dates / money | `Intl.*` with an explicit IANA `timeZone` and ISO currency |
...
Failures name the *cause* — the transitive import that added the bytes — because "bundle too
big" without a chain is not an instruction. Budgets ratchet: the recorded baseline can tighten,
