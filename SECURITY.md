...
| **MCP dev server** | `db.query` is read-only and enforced four ways (SELECT-only role, `BEGIN READ ONLY`, single-statement parse, timeout + row + byte caps); `db.migrate` refuses any database that is not a branch DB. The `/_x` dashboard refuses to mount in production. |
