...
| **MCP tool visibility** | Two orthogonal axes. A tool whose *scope* the caller lacks is refused. A tool hidden from the caller's *role* is omitted from `tools/list` and answers ToolNotFound — never Forbidden, so there is no enumeration behind the curtain. |
